...
class CodeSubmissionServiceTest {
    ...
    private final SubmissionHomeworkContextQuery homework =
            mock(SubmissionHomeworkContextQuery.class);
    ...
    private final CodeSubmissionService service =
            new CodeSubmissionService(
                    ownership,
                    tasks,
                    homework,
                    programs,
    ...
    void arrangeContext() {
                ...
                .thenReturn(
                        Optional.of(
                                new TaskQuery.TaskContext(
                                        ...
                                        TaskType.CODE,
                                        TaskStatus.ACTIVE)));
        when(homework.findSubmissionContext(homeworkItemId))
                .thenReturn(
                        Optional.of(
                                new SubmissionHomeworkContextQuery.HomeworkSubmissionContext(
                                        ...
                                        homeworkItemId,
                                        taskId)));
        when(programs.findStudentProgram(studentProgramId))
                .thenReturn(
                        Optional.of(
        ...
        when(transactions.finish(any(), any(), anyInt(), anyInt(), anyLong(), any(), any()))
                .thenAnswer(
                        invocation ->
                                result(
                                        switch ((CodeExecutionStatus) invocation.getArgument(1)) {
    ...
    void persistsEveryExecutionOutcomeAndUsesServerConfiguration(ExecutionStatus status) {
        ...
        order.verify(transactions)
                .createPending(studentId, studentProgramId, taskId, homeworkItemId, "print(42)", 1);
    ...
    void infrastructureFailureFinalizesExistingSubmissionAsSystemError() {
        ...
        verify(transactions)
                .createPending(studentId, studentProgramId, taskId, homeworkItemId, "pass", 1);
        verify(transactions)
                .finish(
                        any(),
                        eq(CodeExecutionStatus.SYSTEM_ERROR),
                        eq(0),
                        eq(1),
                        eq(0L),
                        isNull(),
