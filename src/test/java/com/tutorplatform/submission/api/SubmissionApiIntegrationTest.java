...
class SubmissionApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void studentSubmitsTextAnswerWithServerOwnedFieldsAndSequentialAttempts() throws Exception {
        ...
        HomeworkEntity homework =
                createHomework(
                        ...
                        List.of(fixture.textTask()));
        UUID itemId = homework.getItems().getFirst().id();
        Instant before = Instant.now();

        submit(
                        fixture.studentPrincipal(),
                        fixture.textTask().getId(),
                        itemId,
        ...
        var attempts =
                submissionRepository.findAttempts(
                        ...
                        0,
                        10);
        assertThat(attempts.items())
                .hasSize(2)
                .allSatisfy(
                        submission -> {
                            assertThat(submission.getStudentId())
                                    .isEqualTo(fixture.student().getId());
                            assertThat(submission.getStudentProgramId())
                                    .isEqualTo(fixture.studentProgram().id());
                            assertThat(submission.getTaskId())
                                    .isEqualTo(fixture.textTask().getId());
        ...
        assertThat(homeworkRepository.findById(homework.getId()))
                .get()
                .extracting(HomeworkEntity::getStatus)
    ...
    void textContractRejectsCodeAndServerOwnedFields() throws Exception {
        ...
        assertThat(
                        submissionRepository
                                .findPageByStudentId(fixture.student().getId(), 0, 10)
                                .items())
                .isEmpty();
    }
    ...
    void attemptsAreNumberedSeparatelyByTaskAndHomeworkItemContext() throws Exception {
        ...
        HomeworkEntity firstHomework =
                createHomework(
                        fixture,
                        fixture.studentProgram(),
                        HomeworkStatus.ASSIGNED,
                        List.of(fixture.textTask(), fixture.secondTextTask()));
        HomeworkEntity secondHomework =
                createHomework(
                        fixture,
                        fixture.studentProgram(),
                        HomeworkStatus.ASSIGNED,
        ...
        submit(
                        ...
                        fixture.textTask().getId(),
                        firstTaskItem,
                        request(firstTaskItem, "A"))
                .andExpect(jsonPath("$.attemptNo").value(1));
        submit(
                        fixture.studentPrincipal(),
                        fixture.secondTextTask().getId(),
                        secondTaskItem,
                        request(secondTaskItem, "B"))
                .andExpect(jsonPath("$.attemptNo").value(1));
        submit(
                        fixture.studentPrincipal(),
                        fixture.textTask().getId(),
                        otherHomeworkItem,
                        request(otherHomeworkItem, "C"))
                .andExpect(jsonPath("$.attemptNo").value(1));
    }
    ...
    void concurrentSubmissionsReceiveDistinctAttemptNumbers() throws Exception {
        ...
        try (var executor = Executors.newFixedThreadPool(2)) {
            ...
            assertThat(responses)
                    .allSatisfy(response -> assertThat(response.getStatus()).isEqualTo(201));
            assertThat(responses)
                    .extracting(
                            response ->
                                    (Integer)
    ...
    @Test
    void foreignAndMismatchedHomeworkContextsAreRejectedWithoutCreatingSubmission()
            throws Exception {
        Fixture owner = createFixture("owner-context");
        Fixture foreign = createFixture("foreign-context");
        HomeworkEntity ownerHomework =
                createHomework(
                        owner,
                        owner.studentProgram(),
                        HomeworkStatus.ASSIGNED,
                        List.of(owner.textTask()));
        HomeworkEntity foreignHomework =
                createHomework(
                        foreign,
                        foreign.studentProgram(),
                        HomeworkStatus.ASSIGNED,
                        List.of(foreign.textTask()));
    ...
    void cancelledHomeworkUnknownTaskAndCodeTaskAreRejected() throws Exception {
        ...
        HomeworkEntity codeHomework =
                createHomework(
                        fixture,
                        fixture.studentProgram(),
                        HomeworkStatus.ASSIGNED,
                        List.of(fixture.codeTask()));
    ...
    void listAndSingleGetAreOwnershipScopedAndPaginated() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            submit(
                            ...
                            owner.textTask().getId(),
                            ownerItem,
                            request(ownerItem, "Owner " + i))
                    .andExpect(status().isCreated());
        }
        String foreignBody =
                submit(
                                foreign.studentPrincipal(),
                                foreign.textTask().getId(),
                                foreignItem,
                                request(foreignItem, "Foreign"))
                        .andReturn()
                        .getResponse()
                        .getContentAsString();
        ...
        mockMvc.perform(
                        get("/api/v1/student/tasks/{taskId}/submissions", owner.textTask().getId())
        ...
        mockMvc.perform(
                        get("/api/v1/student/submissions/{submissionId}", foreignId)
                                .with(user(owner.studentPrincipal())))
                .andExpect(status().isNotFound())
    ...
    void postSecurityRequiresAuthenticationCsrfAndStudentRole() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/v1/student/tasks/{taskId}/submissions", taskId)
                                .with(csrf())
    ...
    void openApiPublishesSubmissionOperationsAndSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/student/tasks/{taskId}/submissions'].post.operationId")
                                .value("submitTextAnswer"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/student/tasks/{taskId}/code-submissions'].post.operationId")
                                .value("submitCodeAnswer"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/student/tasks/{taskId}/submissions'].get.operationId")
                                .value("listStudentTaskSubmissions"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/student/submissions/{submissionId}'].get.operationId")
                                .value("getStudentSubmission"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentSubmissionResponse.properties.id.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentSubmissionResponse.properties.submittedAt.format")
                                .value("date-time"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentSubmissionResponse.properties.status.enum.length()")
                                .value(5))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.SubmitTextAnswerRequest.properties.textAnswer")
                                .exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.SubmitTextAnswerRequest.properties.sourceCode")
                                .doesNotExist())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.SubmitCodeAnswerRequest.properties.sourceCode")
                                .exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.SubmitCodeAnswerRequest.properties.textAnswer")
                                .doesNotExist())
    ...
    void teacherListIsOwnedFilteredPaginatedAndAllowListSorted() throws Exception {
        ...
        saveSubmission(
                fixture, fixture.textTask(), itemId, 1, SubmissionStatus.NEEDS_REVIEW, "A", base);
        saveSubmission(
                fixture,
                fixture.textTask(),
                itemId,
        ...
        statistics.clear();
        mockMvc.perform(
                        get(teacherSubmissionsUrl(fixture.student().getId()))
                ...
                .andExpect(
                        jsonPath("$.items[0].studentId")
                                .value(fixture.student().getId().toString()))
    ...
    void teacherCannotListOrReadForeignStudentSubmission() throws Exception {
        ...
        HomeworkEntity foreignHomework =
                createHomework(
                        ...
                        HomeworkStatus.ASSIGNED,
                        List.of(foreign.textTask()));
        SubmissionEntity foreignSubmission =
                saveSubmission(
                        foreign,
                        foreign.textTask(),
                        foreignHomework.getItems().getFirst().id(),
                        1,
                        SubmissionStatus.NEEDS_REVIEW,
                        "Foreign",
                        Instant.now());
        ...
        mockMvc.perform(
                        get(teacherSubmissionsUrl(foreign.student().getId()))
                                .with(user(owner.teacherPrincipal())))
                .andExpect(status().isNotFound());

        review(
                        owner.teacherPrincipal(),
                        owner.student().getId(),
                        foreignSubmission.getId(),
                        "PASSED",
                        true)
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("SUBMISSION_NOT_FOUND"));
    ...
    void failedReviewDoesNotCompleteHomeworkAndChangesOnlySubmissionStatus() throws Exception {
        ...
        SubmissionEntity passCandidate =
                saveSubmission(
                        fixture,
                        fixture.textTask(),
                        firstItemId,
        ...
        SubmissionEntity failCandidate =
                saveSubmission(
                        fixture,
                        fixture.secondTextTask(),
                        secondItemId,
                        1,
                        SubmissionStatus.NEEDS_REVIEW,
                        "Keep this too",
                        Instant.parse("2026-02-01T00:01:00Z"));
        Integer progressBefore =
                jdbcTemplate.queryForObject(
                        "select count(*) from student_topic_progress", Integer.class);

        review(
                        fixture.teacherPrincipal(),
                        fixture.student().getId(),
                        passCandidate.getId(),
                        "PASSED",
                        true)
        ...
        SubmissionEntity passed =
                submissionRepository.findById(passCandidate.getId()).orElseThrow();
        ...
        HomeworkEntity unchangedHomework =
                homeworkRepository.findById(homework.getId()).orElseThrow();
    ...
    void finalTechnicalAndCodeSubmissionsCannotBeReviewed() throws Exception {
        ...
        for (SubmissionEntity submission : List.of(passed, failed, systemError, code)) {
            ...
            assertThat(submissionRepository.findById(submission.getId()))
                    .get()
                    .extracting(SubmissionEntity::getStatus)
                    .isEqualTo(submission.getStatus());
        }
    ...
    void openApiPublishesTeacherSubmissionOperations() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/submissions'].get.operationId")
                                .value("listTeacherStudentSubmissions"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/submissions/{submissionId}/review'].patch.operationId")
                                .value("reviewTextSubmission"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.TeacherSubmissionResponse.properties.id.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.TeacherSubmissionResponse.properties.status.enum.length()")
                                .value(5))
                .andExpect(jsonPath("$.components.schemas.ApiError").exists());
    }

    private ResultActions submit(
            AuthenticatedUser principal, UUID taskId, UUID homeworkItemId, String body)
            throws Exception {
    ...
    private TaskEntity createTask(
            TeacherEntity teacher, SubjectEntity subject, TaskType type, String title) {
