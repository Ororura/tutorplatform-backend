...
class SubmissionReviewedHomeworkListenerTest {
    ...
    private final HomeworkCompletionService completionService = mock(HomeworkCompletionService.class);
    ...
    void repeatedEventHandlingIsSafeToDelegate() {
        ...
        verify(completionService, times(2)).recalculate(
            event.homeworkItemId(), event.studentId(), event.studentProgramId(), event.taskId()
        );
    }
    ...
    void completionFailureIsIsolatedFromAfterCommitEventHandling() {
        SubmissionReviewedEvent event = event(UUID.randomUUID(), SubmissionStatus.PASSED);
        doThrow(new IllegalStateException("completion failed")).when(completionService).recalculate(
            event.homeworkItemId(), event.studentId(), event.studentProgramId(), event.taskId()
        );
