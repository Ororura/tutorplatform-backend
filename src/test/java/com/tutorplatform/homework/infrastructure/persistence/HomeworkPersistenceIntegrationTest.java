...
class HomeworkPersistenceIntegrationTest extends PostgresIntegrationTest {
    ...
    void homeworkIsSavedTogetherWithItems() {
        ...
        HomeworkEntity homework =
                saveHomework(fixture, HomeworkStatus.ASSIGNED, null, twoItems(fixture));

        HomeworkEntity persisted =
                homeworkRepository.findByIdWithItems(homework.getId()).orElseThrow();
        ...
        assertThat(persisted.getItems())
                .extracting(HomeworkItemEntity::taskId)
    ...
    void homeworkOptimisticLockingWorks() {
        ...
        HomeworkEntity stale =
                transaction.execute(
                        status -> homeworkRepository.findById(saved.getId()).orElseThrow());

        first.update(
                "Первая версия", first.getDescription(), first.getDueAt(), first.getStatus(), null);
        transaction.executeWithoutResult(status -> homeworkRepository.saveAndFlush(first));

        stale.update(
                "Устаревшая версия",
                stale.getDescription(),
                stale.getDueAt(),
                stale.getStatus(),
                null);
        Throwable thrown =
        ...
        assertThat(homeworkRepository.findById(saved.getId()))
                .get()
                .extracting(HomeworkEntity::getTitle)
    ...
    void homeworkItemIsSavedAndCanBeFoundInHomeworkContext() {
        ...
        HomeworkEntity homework =
                saveHomework(fixture, HomeworkStatus.ASSIGNED, null, List.of(item));
    ...
    void deletingHomeworkCascadesHomeworkItems() {
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "select count(*) from homework_items where homework_id = ?",
                                Integer.class,
                                homework.getId()))
                .isZero();
    }
    ...
    void deletingHomeworkItemDoesNotDeleteTask() {
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "select count(*) from homework_items where id = ?",
                                Integer.class,
                                itemId))
                .isZero();
    }
    ...
    void homeworkListIsFilteredByStudentProgramAndUsesDatabasePagination() {
        ...
        assertThat(
                        homeworkRepository.findAllByStudentProgramId(
                                fixture.studentProgram().id(), 0, 1))
                .extracting(HomeworkEntity::getStudentProgramId)
                .containsExactly(fixture.studentProgram().id());
        assertThat(
                        homeworkRepository.existsByIdAndStudentProgramId(
    ...
    void submissionsSchemaMatchesErModelConstraints() {
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "select text_answer from submissions where id = ?",
                                String.class,
                                submissionId))
                .isEqualTo("Ответ");
    ...
    void codeSubmissionsSchemaMatchesErModelConstraints() {
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "select source_code from code_submissions where submission_id = ?",
                                String.class,
                                submissionId))
                .isEqualTo("print(1)");
        assertThatThrownBy(
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "select count(*) from code_submissions where submission_id = ?",
                                Integer.class,
                                submissionId))
                .isZero();
    }
    ...
    private UUID insertSubmission(
            HomeworkFixture fixture, UUID homeworkItemId, int attemptNo, String status) {
