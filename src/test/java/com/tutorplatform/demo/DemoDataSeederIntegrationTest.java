...
class DemoDataSeederIntegrationTest extends PostgresIntegrationTest {
    ...
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_demo_data_seeder", null);
    }
    ...
    void credentialsTokensAndDomainConsistencyAreValid() {
        ...
        assertThat(
            ...
            """
                                        .formatted(ALEX_COMPLETED_PERIOD)))
                .isEqualTo(300);
        assertThat(
                        queryInt(
                                """
    ...
    void realAuthenticationAndCoreApiFlowsWork() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/students/{id}/progress", ALEX)
                                .param("studentProgramId", ALEX_PROGRAM.toString())
                                .cookie(teacherSession))
                .andExpect(status().isOk())
    ...
    void ilyaCanBeAssignedExistingTemplateThroughPublicTeacherApi() throws Exception {
        ...
        MvcResult templates =
                mockMvc.perform(get("/api/v1/teacher/programs").cookie(teacherSession))
                        .andExpect(status().isOk())
                        .andReturn();
        JsonNode templateList =
                objectMapper.readTree(templates.getResponse().getContentAsByteArray());
        ...
        MvcResult assigned =
                mockMvc.perform(
                                post("/api/v1/teacher/students/{id}/programs", ILYA)
                                        .cookie(csrf.cookie())
                                        .header(csrf.headerName(), csrf.token())
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(
                                                "{\"learningProgramId\":\""
                                                        + selected.required("id").textValue()
                                                        + "\"}"))
        ...
        String studentProgramId =
                objectMapper
                        .readTree(assigned.getResponse().getContentAsByteArray())
                        .required("id")
                        .textValue();
        ...
        mockMvc.perform(
                        get(
                                        "/api/v1/teacher/students/{studentId}/programs/{id}",
                                        ILYA,
                                        studentProgramId)
                                .cookie(teacherSession))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.modules.length()").value(org.hamcrest.Matchers.greaterThan(0)));
    }
    ...
    private CsrfExchange obtainCsrf() throws Exception {
        ...
        return new CsrfExchange(
                json.required("headerName").textValue(),
                json.required("token").textValue(),
