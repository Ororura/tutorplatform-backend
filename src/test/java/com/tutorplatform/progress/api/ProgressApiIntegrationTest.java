...
@Transactional
class ProgressApiIntegrationTest extends PostgresIntegrationTest {

    @DynamicPropertySource
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_progress_api", "008");
    }
    ...
    void teacherAndStudentEndpointsReturnTheSameCompleteProgressWithoutCsrf() throws Exception {
        ...
        MvcResult teacherResult =
                mockMvc.perform(
                                get(teacherUrl(fixture))
                        ...
                        .andExpect(
                                jsonPath("$.topics.completed[0].id")
                                        .value(completedTopic.toString()))
                        ...
                        .andExpect(
                                jsonPath("$.topics.inProgress[0].id")
                                        .value(inProgressTopic.toString()))
    ...
    @Test
    void explicitProgramIdSelectsBetweenMultipleProgramsAndCannotBeOverriddenByClientIds()
            throws Exception {
    ...
    void openApiPublishesProgressOperationsSchemasAndRequiredProgramParameter() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/progress'].get.operationId")
                                .value("getTeacherStudentProgress"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/student/progress'].get.operationId")
                                .value("getCurrentStudentProgress"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/student/progress'].get.parameters[0].name")
                                .value("studentProgramId"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/student/progress'].get.parameters[0].required")
                                .value(true))
                .andExpect(jsonPath("$.components.schemas.CurrentProgressResponse").exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.CurrentProgressResponse.properties.assessment")
                                .exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.AssessmentResponse.properties.understandingAverage.type[1]")
                                .value("null"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.AssessmentResponse.properties.practiceAverage.type[1]")
                                .value("null"))
    ...
    private Fixture fixture(String suffix) {
        ...
        jdbc.update(
                "INSERT INTO users(id, email) VALUES (?, ?)",
                studentUserId,
                suffix + "-student-" + studentUserId + "@example.com");
        jdbc.update(
    ...
    private UUID module(Fixture fixture, int position) {
        ...
        jdbc.update(
                "INSERT INTO modules(id, learning_program_id, title, position) VALUES (?, ?, 'Module', ?)",
                id,
