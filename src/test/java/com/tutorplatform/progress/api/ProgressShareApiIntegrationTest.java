...
class ProgressShareApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void teacherCreatesSecureSharesAndListsOnlyMetadata() throws Exception {
        ...
        MvcResult createdResult =
                create(fixture, fixture.studentProgramId(), expiresAt)
        ...
        String storedHash =
                jdbc.queryForObject(
                        "select token_hash from progress_shares where id = ?",
                        String.class,
                        shareId);
        ...
        assertThat(
                        jdbc.queryForObject(
                                "select created_by_teacher_id from progress_shares where id = ?",
                                UUID.class,
                                shareId))
        ...
        MvcResult second =
                create(fixture, fixture.studentProgramId(), null)
                        .andExpect(status().isCreated())
    ...
    void createValidatesOwnershipProgramExpiryAuthenticationAndCsrf() throws Exception {
        ...
        mockMvc.perform(
                        post(sharesUrl(foreign))
                                .with(user(owner.principal()))
                                .with(csrf())
                                .contentType("application/json")
                                .content(
                                        request(
                                                foreign.studentProgramId(),
        ...
        mockMvc.perform(
                        post(sharesUrl(owner))
                                .with(user(owner.principal()))
                                .contentType("application/json")
                                .content(request(owner.studentProgramId(), null)))
    ...
    void listComputesStatusesWithRevokedPrecedenceAndScopesTeacher() throws Exception {
        ...
        UUID revoked =
                share(fixture, "revoked-token", Instant.now().minusSeconds(10), Instant.now());
    ...
    void revokeIsIdempotentPersistentAndOwnershipScoped() throws Exception {
        ...
        Instant revokedAt =
                jdbc.queryForObject(
                        "select revoked_at from progress_shares where id = ?",
                        Instant.class,
                        shareId);
        ...
        assertThat(
                        jdbc.queryForObject(
                                "select count(*) from progress_shares where id = ?",
                                Integer.class,
                                shareId))
                .isOne();
    ...
    void publicTokenReturnsLiveParentSafeProgressWithoutSessionOrCsrf() throws Exception {
        ...
        MvcResult first =
                mockMvc.perform(get("/api/v1/public/progress/{token}", rawToken))
                        .andExpect(status().isOk())
    ...
    void openApiPublishesShareOperationsAndPublicProjection() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/progress/shares'].post.operationId")
                                .value("createProgressShare"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/progress/shares'].get.operationId")
                                .value("listProgressShares"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/progress/shares/{shareId}'].delete.operationId")
                                .value("revokeProgressShare"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/public/progress/{token}'].get.operationId")
                                .value("getPublicCurrentProgress"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/public/progress/{token}'].get.security")
                                .isEmpty())
                ...
                .andExpect(
                        jsonPath("$.components.schemas.ProgressShareStatus.enum.length()").value(3))
                .andExpect(jsonPath("$.components.schemas.PublicCurrentProgressResponse").exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.PublicCurrentProgressResponse.properties.studentProgramId")
                                .doesNotExist())
                .andExpect(
                        jsonPath("$.components.schemas.PublicTopicResponse.properties.id")
                                .doesNotExist())
                .andExpect(jsonPath("$.components.schemas.ApiError").exists());
    }

    private org.springframework.test.web.servlet.ResultActions create(
            Fixture fixture, UUID studentProgramId, Instant expiresAt) throws Exception {
        return mockMvc.perform(
                post(sharesUrl(fixture))
                        .with(user(fixture.principal()))
                        .with(csrf())
                        .contentType("application/json")
                        .content(request(studentProgramId, expiresAt)));
    }

    private org.springframework.test.web.servlet.ResultActions revoke(Fixture fixture, UUID shareId)
            throws Exception {
        return mockMvc.perform(
                delete(sharesUrl(fixture) + "/" + shareId)
                        .with(user(fixture.principal()))
                        .with(csrf()));
    }
    ...
    private String request(UUID studentProgramId, Instant expiresAt) {
        var request =
                objectMapper
                        .createObjectNode()
                        .put("studentProgramId", studentProgramId.toString());
        if (expiresAt != null) {
    ...
    private record Fixture(
            UUID teacherUserId, UUID teacherId, UUID studentId, UUID studentProgramId) {
        private AuthenticatedUser principal() {
            return new AuthenticatedUser(
                    teacherUserId,
                    teacherUserId + "@example.com",
                    "hash",
                    true,
