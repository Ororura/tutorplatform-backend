...
class TeacherProgramManagementApiIntegrationTest extends PostgresIntegrationTest {
    ...
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(
                registry, "test_teacher_program_management_api", null);
    }
    ...
    void subjectsReturnSystemAndOwnButNotForeignAndDefaultToActive() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/subjects?status=ARCHIVED")
    ...
    void getsOwnedProgramBySlugAndHidesForeignPrograms() throws Exception {
        ...
        LearningProgramEntity own =
                program(teacher.teacher, "Питон с нуля", LearningProgramStatus.DRAFT);

        program(foreign.teacher, "Чужая программа", LearningProgramStatus.DRAFT);
    ...
    void archivesDraftActiveAndArchivedProgramsAndIsIdempotent() throws Exception {
        ...
        LearningProgramEntity draft =
                program(teacher.teacher, "Draft", LearningProgramStatus.DRAFT);
        LearningProgramEntity active =
                program(teacher.teacher, "Active", LearningProgramStatus.ACTIVE);
    ...
    void archiveHidesForeignProgramAndPreservesExistingAssignment() throws Exception {
        ...
        LearningProgramEntity foreignProgram =
                program(foreign.teacher, "Foreign", LearningProgramStatus.ACTIVE);
        StudentEntity student = student(teacher.teacher, "Assigned student");
        StudentProgramEntity assignment =
        ...
        mockMvc.perform(
                        post("/api/v1/teacher/programs/{programId}/archive", foreignProgram.getId())
                                .with(user(teacher.principal))
                                .with(csrf()))
                .andExpect(status().isNotFound())
        ...
        StudentProgramEntity persisted =
                studentProgramRepository.findById(assignment.id()).orElseThrow();
        ...
        expectAssign(
                ...
                null,
                409,
                "LEARNING_PROGRAM_STATUS_CONFLICT");
    }

    private void archive(TeacherContext teacher, LearningProgramEntity program) throws Exception {
        mockMvc.perform(
                        post("/api/v1/teacher/programs/{programId}/archive", program.getId())
                                .with(user(teacher.principal))
                                .with(csrf()))
                .andExpect(status().isOk())
    ...
    void createListAndActivateOwnTemplateWithAuthorizedSubjects() throws Exception {
        ...
        SubjectEntity foreignSubject =
                subject(foreign.teacher.id(), "Foreign subject", SubjectStatus.ACTIVE);
        program(foreign.teacher, "Foreign template", LearningProgramStatus.DRAFT);

        String body =
                objectMapper.writeValueAsString(
                        new CreateLearningProgramRequest(
        ...
        mockMvc.perform(
                        post("/api/v1/teacher/programs")
                                ...
                                .content(
                                        objectMapper.writeValueAsString(
                                                new CreateLearningProgramRequest(
                                                        PYTHON, "System Python", null))))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.subject.code").value("PYTHON"));

        mockMvc.perform(
                        post("/api/v1/teacher/programs/{id}/activate", programId)
                                .with(user(teacher.principal))
                                .with(csrf()))
                .andExpect(status().isOk())
    ...
    void getsOwnedProgramDetailsWithOrderedStructureAndAssignmentFlags() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Details", LearningProgramStatus.ACTIVE);
        ModuleEntity laterModule =
        ...
        ModuleEntity firstModule =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(
                                ...
                                "First description",
                                1));
        TopicEntity laterTopic =
                topicRepository.saveAndFlush(
                        new TopicEntity(
                                UUID.randomUUID(),
                                firstModule.id(),
                                "Later topic",
                                "Later topic description",
                                3,
                                TopicStatus.DRAFT));
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/programs/{programId}", program.getId())
                                .with(user(teacher.principal)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(program.getId().toString()))
                .andExpect(jsonPath("$.subject.id").value(PYTHON.toString()))
    ...
    void doesNotExposeForeignOrMissingProgramDetails() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/programs/{programId}", foreignProgram.getId())
                                .with(user(teacher.principal)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("LEARNING_PROGRAM_NOT_FOUND"));
        mockMvc.perform(
                        get("/api/v1/teacher/programs/{programId}", UUID.randomUUID())
                                .with(user(teacher.principal)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("LEARNING_PROGRAM_NOT_FOUND"));
    }
    ...
    void detailsAreEditableOnlyForUnassignedNonArchivedProgram() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/programs/{programId}", draft.getId())
                                .with(user(teacher.principal)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.hasAssignments").value(false))
                .andExpect(jsonPath("$.editable").value(true));
        mockMvc.perform(
                        get("/api/v1/teacher/programs/{programId}", archived.getId())
                                .with(user(teacher.principal)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.hasAssignments").value(false))
                .andExpect(jsonPath("$.editable").value(false));
    ...
    void patchUpdatesOwnedDraftAndActiveProgramsWithoutChangingSubjectOrStatus() throws Exception {
        ...
        mockMvc.perform(
                        patch("/api/v1/teacher/programs/{programId}", draft.getId())
                                .with(user(teacher.principal))
                                .with(csrf())
                                .contentType("application/json")
                                .content(
        ...
        mockMvc.perform(
                        patch("/api/v1/teacher/programs/{programId}", active.getId())
                                .with(user(teacher.principal))
                                .with(csrf())
                                .contentType("application/json")
                                .content(
                                        updateRequest("Updated active", null, active.getVersion())))
                .andExpect(status().isOk())
    ...
    void patchHidesForeignProgram() throws Exception {
        ...
        mockMvc.perform(
                        patch("/api/v1/teacher/programs/{programId}", program.getId())
    ...
    void patchValidatesTrimmedTitleAndVersion() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Valid", LearningProgramStatus.DRAFT);
    ...
    void stalePatchReturnsVersionConflict() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Initial", LearningProgramStatus.DRAFT);
    ...
    void assignmentIsOwnedActiveAtomicVisibleAndInitializesLockedProgress() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Python template", LearningProgramStatus.ACTIVE);
        ModuleEntity module =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), program.getId(), "M", null, 0));
        TopicEntity topic =
                topicRepository.saveAndFlush(
                        new TopicEntity(
                                UUID.randomUUID(), module.id(), "T", null, 0, TopicStatus.ACTIVE));

        String body = "{\"learningProgramId\":\"" + program.getId() + "\"}";
        String response =
                mockMvc.perform(
                                post("/api/v1/teacher/students/{id}/programs", student.getId())
                                        .with(user(teacher.principal))
                                        .with(csrf())
                                        .contentType("application/json")
                                        .content(body))
        ...
        UUID studentProgramId =
                UUID.fromString(objectMapper.readTree(response).get("id").textValue());
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/students/{id}/programs", student.getId())
                                .with(user(teacher.principal)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(studentProgramId.toString()));
        mockMvc.perform(
                        get(
                                        "/api/v1/teacher/students/{studentId}/programs/{programId}",
                                        student.getId(),
                                        studentProgramId)
                                .with(user(teacher.principal)))
                .andExpect(status().isOk())
    ...
    @Test
    void assignmentRejectsForeignResourcesDraftArchivedAndInvalidIntervalButAllowsDifferentProgram()
            throws Exception {
        ...
        LearningProgramEntity active =
                program(teacher.teacher, "Active", LearningProgramStatus.ACTIVE);
        LearningProgramEntity second =
                program(teacher.teacher, "Second", LearningProgramStatus.ACTIVE);
        LearningProgramEntity draft =
                program(teacher.teacher, "Draft", LearningProgramStatus.DRAFT);
        LearningProgramEntity archived =
                program(teacher.teacher, "Archived", LearningProgramStatus.ARCHIVED);
        ...
        expectAssign(
                teacher, foreignStudent.getId(), active.getId(), null, 404, "STUDENT_NOT_FOUND");
        expectAssign(
                teacher,
                ownStudent.getId(),
                foreignProgram.getId(),
                null,
                404,
                "LEARNING_PROGRAM_NOT_FOUND");
        expectAssign(
                teacher,
                ownStudent.getId(),
                draft.getId(),
    ...
    @Test
    void createsFirstModuleAndAppendsSequentialModulesWithoutChangingExistingOnes()
            throws Exception {
        TeacherContext teacher = teacher("module-create@example.com");
        LearningProgramEntity program =
                program(teacher.teacher, "Modules", LearningProgramStatus.DRAFT);

        String firstResponse =
                createModule(teacher, program.getId(), "  Основы Python  ", "  Первый раздел  ")
                        .andExpect(status().isCreated())
                        .andExpect(header().exists("Location"))
    ...
    void createModuleRejectsForeignArchivedAndAssignedPrograms() throws Exception {
        ...
        LearningProgramEntity archived =
                program(owner.teacher, "Archived", LearningProgramStatus.ARCHIVED);
        LearningProgramEntity assigned =
                program(owner.teacher, "Assigned", LearningProgramStatus.ACTIVE);
    ...
    void createsDraftTopicAppendedToModuleWithJpaVersion() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Topics", LearningProgramStatus.DRAFT);
        ModuleEntity module =
                moduleRepository.saveAndFlush(
    ...
    @Test
    void createTopicRejectsInvalidTitlesAndForeignMismatchedOrNonEditableParents()
            throws Exception {
        ...
        ModuleEntity module =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), owned.getId(), "Module", null, 0));
        ModuleEntity archivedModule =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), archived.getId(), "Archived", null, 0));
    ...
    void updatesTopicInPlaceWithTrimmedFieldsStatusAndJpaVersion() throws Exception {
        ...
        ModuleEntity module =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), program.getId(), "Module", null, 4));
        TopicEntity topic =
                topicRepository.saveAndFlush(
                        new TopicEntity(
                                UUID.randomUUID(),
    ...
    @Test
    void updateTopicHidesForeignAndMismatchedResourcesAndRejectsNonEditableProgram()
            throws Exception {
        ...
        ModuleEntity module =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), owned.getId(), "Module", null, 0));
        ModuleEntity otherModule =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), other.getId(), "Other", null, 0));
        ModuleEntity archivedModule =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), archived.getId(), "Archived", null, 0));
        ModuleEntity assignedModule =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), assigned.getId(), "Assigned", null, 0));
        TopicEntity topic =
                topicRepository.saveAndFlush(
                        new TopicEntity(
                                UUID.randomUUID(),
        ...
        updateTopic(
                        ...
                        null,
                        TopicStatus.ACTIVE,
                        assignedTopic.version())
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("LEARNING_PROGRAM_STATUS_CONFLICT"));
        assertThat(
                        progressRepository
                                .findById(assignment.id(), assignedTopic.id())
                                .orElseThrow()
                                .status())
    ...
    void updateTopicValidatesRequestAndRejectsStaleVersion() throws Exception {
        ...
        for (String body :
                List.of(
                        ...
                        topicUpdateRequest(
                                "x".repeat(181), null, TopicStatus.ACTIVE, topic.version()),
    ...
    void reorderModulesRejectsMissingForeignDuplicateAndEmptyIdsAtomically() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Modules", LearningProgramStatus.DRAFT);
        LearningProgramEntity other =
                program(teacher.teacher, "Other", LearningProgramStatus.DRAFT);
        ModuleEntity first = module(program, "First", 0);
        ModuleEntity second = module(program, "Second", 1);
        ...
        for (List<UUID> invalidOrder :
            ...
            assertThat(moduleRepository.findById(second.id()).orElseThrow().position())
                    .isEqualTo(1);
        }
    }
    ...
    void reorderModulesHidesProgramsOwnedByAnotherTeacher() throws Exception {
        ...
        LearningProgramEntity program =
                program(owner.teacher, "Modules", LearningProgramStatus.DRAFT);
        ModuleEntity first = module(program, "First", 0);
    ...
    void reorderModulesRejectsAssignedProgram() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Modules", LearningProgramStatus.ACTIVE);
        ModuleEntity first = module(program, "First", 0);
        ModuleEntity second = module(program, "Second", 1);
        StudentEntity student = student(teacher.teacher, "Student");
        studentProgramRepository.saveAndFlush(
    ...
    void reorderTopicsRejectsAssignedProgram() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Topics", LearningProgramStatus.ACTIVE);
        ModuleEntity module = module(program, "Module", 0);
        TopicEntity first = topic(module, "First", 0);
        TopicEntity second = topic(module, "Second", 1);
        StudentEntity student = student(teacher.teacher, "Student");
    ...
    void reorderTopicsRejectsArchivedProgram() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Topics", LearningProgramStatus.ARCHIVED);
        ModuleEntity module = module(program, "Module", 0);
        TopicEntity first = topic(module, "First", 0);
        TopicEntity second = topic(module, "Second", 1);
    ...
    @Test
    void moduleMutationHidesForeignAndMismatchedModulesAndRejectsNonEditablePrograms()
            throws Exception {
    ...
    void deletesOnlyEmptyModuleWithoutNormalizingOtherPositions() throws Exception {
        ...
        LearningProgramEntity program =
                program(teacher.teacher, "Modules", LearningProgramStatus.DRAFT);
        ModuleEntity deleted =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), program.getId(), "Deleted", null, 0));
        ModuleEntity retained =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(UUID.randomUUID(), program.getId(), "Retained", null, 3));
        ModuleEntity withTopic =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(
                                UUID.randomUUID(), program.getId(), "With topic", null, 5));
        TopicEntity topic =
                topicRepository.saveAndFlush(
                        new TopicEntity(
                                UUID.randomUUID(),
        ...
        deleteModule(teacher, program.getId(), deleted.id()).andExpect(status().isNoContent());
    ...
    void openApiPublishesStableManagementOperationIds() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/programs'].get.operationId")
                                .value("listTeacherLearningPrograms"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/programs'].post.operationId")
                                .value("createTeacherLearningProgram"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules'].post.operationId")
                                .value("createTeacherLearningProgramModule"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules/order'].put.operationId")
                                .value("reorderTeacherLearningProgramModules"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules/{moduleId}/topics'].post.operationId")
                                .value("createTeacherLearningProgramTopic"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules/{moduleId}/topics/order'].put.operationId")
                                .value("reorderTeacherLearningProgramTopics"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules/{moduleId}/topics/{topicId}'].patch.operationId")
                                .value("updateTeacherLearningProgramTopic"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules/{moduleId}'].patch.operationId")
                                .value("updateTeacherLearningProgramModule"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/modules/{moduleId}'].delete.operationId")
                                .value("deleteTeacherLearningProgramModule"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/programs/{programId}'].get.operationId")
                                .value("getTeacherLearningProgram"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}'].patch.operationId")
                                .value("updateTeacherLearningProgram"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/activate'].post.operationId")
                                .value("activateTeacherLearningProgram"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/programs/{programId}/archive'].post.operationId")
                                .value("archiveTeacherLearningProgram"))
                .andExpect(
                        jsonPath(
    ...
    private void expectAssign(
        ...
        var action =
                mockMvc.perform(
                        post("/api/v1/teacher/students/{id}/programs", studentId)
                                .with(user(teacher.principal))
                                .with(csrf())
                                .contentType("application/json")
                                .content(
    ...
    private org.springframework.test.web.servlet.ResultActions createModule(
            TeacherContext teacher, UUID programId, String title, String description)
            throws Exception {
        return mockMvc.perform(
    ...
    private org.springframework.test.web.servlet.ResultActions updateModule(
        ...
        return mockMvc.perform(
                patch(
                        ...
                        .content(
                                objectMapper.writeValueAsString(
                                        new UpdateLearningProgramModuleRequest(
                                                title, description))));
    }

    private org.springframework.test.web.servlet.ResultActions reorderModules(
            TeacherContext teacher, UUID programId, List<UUID> orderedIds) throws Exception {
        return mockMvc.perform(
                put("/api/v1/teacher/programs/{programId}/modules/order", programId)
                        .with(user(teacher.principal))
                        .with(csrf())
                        .contentType("application/json")
                        .content(
                                objectMapper.writeValueAsString(
                                        new ReorderLearningProgramModulesRequest(orderedIds))));
    }
    ...
    private org.springframework.test.web.servlet.ResultActions reorderTopics(
        ...
        return mockMvc.perform(
                put(
                        ...
                        .content(
                                objectMapper.writeValueAsString(
                                        new ReorderLearningProgramTopicsRequest(orderedIds))));
    }
    ...
    private org.springframework.test.web.servlet.ResultActions deleteModule(
            TeacherContext teacher, UUID programId, UUID moduleId) throws Exception {
        return mockMvc.perform(
                delete(
                                "/api/v1/teacher/programs/{programId}/modules/{moduleId}",
                                programId,
    ...
    private void expectUpdateConflict(
            TeacherContext teacher, LearningProgramEntity program, String code) throws Exception {
        mockMvc.perform(
                        patch("/api/v1/teacher/programs/{programId}", program.getId())
                                .with(user(teacher.principal))
                                .with(csrf())
    ...
    private String updateRequest(String title, String description, Long version) throws Exception {
        return objectMapper.writeValueAsString(
                new UpdateLearningProgramRequest(title, description, version));
    }

    private String topicUpdateRequest(
            String title, String description, TopicStatus status, Long version) throws Exception {
        return objectMapper.writeValueAsString(
                new UpdateLearningProgramTopicRequest(title, description, status, version));
    }
    ...
    private StudentEntity student(TeacherEntity teacher, String name) {
        StudentEntity student =
                studentRepository.saveAndFlush(
                        new StudentEntity(UUID.randomUUID(), name, null, StudentStatus.ACTIVE));
    ...
    private SubjectEntity subject(UUID ownerId, String name, SubjectStatus status) {
        return subjectRepository.saveAndFlush(
                new SubjectEntity(UUID.randomUUID(), ownerId, null, name, null, status));
    }

    private LearningProgramEntity program(
            TeacherEntity teacher, String title, LearningProgramStatus status) {
        return learningProgramRepository.saveAndFlush(
    ...
    private ModuleEntity module(LearningProgramEntity program, String title, int position) {
        return moduleRepository.saveAndFlush(
                new ModuleEntity(UUID.randomUUID(), program.getId(), title, null, position));
    }
