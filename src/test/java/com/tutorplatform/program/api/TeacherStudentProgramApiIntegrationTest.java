...
class TeacherStudentProgramApiIntegrationTest extends PostgresIntegrationTest {
    ...
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_teacher_student_program_api", null);
    }

    private static final UUID PYTHON_SUBJECT_ID = UUID.fromString("6513554d-dceb-5902-b6df-557c7a94b5c7");
    ...
    @Test
    void listReturnsAllProgramsInDeterministicOrderWithSubjectAndWithoutStructure() throws Exception {
        ...
        StudentProgramEntity older = createProgram(teacher.teacher(), student, "Первая программа", now.minus(2, ChronoUnit.DAYS));
        StudentProgramEntity newer = createProgram(teacher.teacher(), student, "Текущая программа", now.minus(1, ChronoUnit.DAYS));

        MvcResult result = mockMvc.perform(get(programsUrl(student.getId()))
    ...
    @Test
    void listReturnsEmptyArrayForOwnedStudentWithoutProgramsAndDoesNotRequireCsrf() throws Exception {
    ...
    void endpointsRequireAuthenticationAndTeacherRole() throws Exception {
        ...
        mockMvc.perform(get(programsUrl(studentId)))
    ...
    void detailReturnsOrderedHierarchyLifecycleAndEveryProgressStatus() throws Exception {
        ...
        StudentEntity student = createStudent(teacher.teacher(), "Алексей");
        StudentProgramEntity program = createProgram(
            teacher.teacher(), student, "Python с нуля", Instant.now().minus(10, ChronoUnit.DAYS)
        );
        LearningProgramEntity learningProgram = learningProgramRepository.findById(program.learningProgramId()).orElseThrow();
        ...
        mockMvc.perform(get(programUrl(student.getId(), program.id())).with(user(teacher.principal())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(program.id().toString()))
            .andExpect(jsonPath("$.learningProgramId").value(learningProgram.getId().toString()))
    ...
    void detailReturnsNullWhenTopicProgressRowIsMissing() throws Exception {
        ...
        StudentProgramEntity program = createProgram(teacher.teacher(), student, "Начало", Instant.now());
        LearningProgramEntity learningProgram = learningProgramRepository.findById(program.learningProgramId()).orElseThrow();
        ...
        mockMvc.perform(get(programUrl(student.getId(), program.id())).with(user(teacher.principal())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.modules[0].topics[0].progressStatus").value(org.hamcrest.Matchers.nullValue()));
    }
    ...
    void detailRejectsProgramFromAnotherStudentAndUnknownProgram() throws Exception {
        ...
        StudentProgramEntity secondProgram = createProgram(teacher.teacher(), secondStudent, "Вторая", Instant.now());

        mockMvc.perform(get(programUrl(firstStudent.getId(), secondProgram.id())).with(user(teacher.principal())))
            .andExpect(status().isNotFound())
            .andExpect(jsonPath("$.code").value("STUDENT_PROGRAM_NOT_FOUND"));

        mockMvc.perform(get(programUrl(firstStudent.getId(), UUID.randomUUID())).with(user(teacher.principal())))
            .andExpect(status().isNotFound())
            .andExpect(jsonPath("$.code").value("STUDENT_PROGRAM_NOT_FOUND"));
    }
    ...
    void foreignTeacherCannotReadStudentProgram() throws Exception {
        ...
        StudentEntity student = createStudent(owner.teacher(), "Чужой ученик");
        StudentProgramEntity program = createProgram(owner.teacher(), student, "Скрытая", Instant.now());

        mockMvc.perform(get(programUrl(student.getId(), program.id())).with(user(foreignTeacher.principal())))
            .andExpect(status().isNotFound())
            .andExpect(jsonPath("$.code").value("STUDENT_NOT_FOUND"));
    }
    ...
    void topicIdFromDetailOpensExistingMaterialsApi() throws Exception {
        ...
        StudentEntity student = createStudent(teacher.teacher(), "Алексей");
        StudentProgramEntity program = createProgram(teacher.teacher(), student, "Материалы", Instant.now());
        LearningProgramEntity learningProgram = learningProgramRepository.findById(program.learningProgramId()).orElseThrow();
        ModuleEntity module = createModule(learningProgram, "Модуль", 0);
        TopicEntity topic = createTopic(module, "Тема", 0, TopicStatus.ACTIVE);
        materialRepository.saveAndFlush(new LessonMaterialEntity(
            UUID.randomUUID(), topic.id(), teacher.teacher().id(), LessonMaterialType.TEXT,
            "Конспект", "Текст материала", null, null, 0
        ));

        MvcResult detail = mockMvc.perform(get(programUrl(student.getId(), program.id()))
                .with(user(teacher.principal())))
            .andExpect(status().isOk())
            .andReturn();
        String topicId = json(detail).at("/modules/0/topics/0/id").textValue();

        mockMvc.perform(get("/api/v1/teacher/topics/{topicId}/materials", topicId)
                .with(user(teacher.principal())))
            .andExpect(status().isOk())
    ...
    void openApiPublishesProgramOperationsAndSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.paths['/api/v1/teacher/students/{studentId}/programs'].get.operationId")
                .value("listTeacherStudentPrograms"))
            .andExpect(jsonPath("$.paths['/api/v1/teacher/students/{studentId}/programs/{studentProgramId}'].get.operationId")
                .value("getTeacherStudentProgram"))
            .andExpect(jsonPath("$.paths['/api/v1/teacher/students/{studentId}/programs/{studentProgramId}'].get.parameters[0].schema.format")
                .value("uuid"))
            .andExpect(jsonPath("$.components.schemas.StudentProgramDetailsResponse.properties.startedAt.format")
                .value("date-time"))
            .andExpect(jsonPath("$.components.schemas.ProgramTopicResponse.properties.topicStatus.enum.length()")
                .value(3))
            .andExpect(jsonPath("$.components.schemas.ProgramTopicResponse.properties.progressStatus.enum.length()")
                .value(4));
    }
    ...
    private StudentProgramEntity createProgram(
        ...
        LearningProgramEntity learningProgram = learningProgramRepository.saveAndFlush(new LearningProgramEntity(
            UUID.randomUUID(), teacher.id(), subject.id(), title, "Описание " + title, LearningProgramStatus.ACTIVE
        ));
    ...
    private ModuleEntity createModule(LearningProgramEntity learningProgram, String title, int position) {
        return moduleRepository.saveAndFlush(new ModuleEntity(
            UUID.randomUUID(), learningProgram.getId(), title, "Описание " + title, position
        ));
    }

    private TopicEntity createTopic(ModuleEntity module, String title, int position, TopicStatus status) {
        return topicRepository.saveAndFlush(new TopicEntity(
            UUID.randomUUID(), module.id(), title, "Описание " + title, position, status
        ));
    }
    ...
    private void createProgress(
        ...
        Instant startedAt = status == StudentTopicProgressStatus.IN_PROGRESS
            || status == StudentTopicProgressStatus.COMPLETED ? Instant.now() : null;
