...
class StudentProgramApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void listReturnsOnlyCurrentStudentsPrograms() throws Exception {
        ...
        StudentProgramEntity older =
                createProgram(owner, "Первая программа", now.minus(2, ChronoUnit.DAYS));
        StudentProgramEntity newer =
                createProgram(owner, "Текущая программа", now.minus(1, ChronoUnit.DAYS));
    ...
    void detailReturnsOwnProgramWithOrderedModulesAndTopics() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/student/programs/{studentProgramId}", program.id())
    ...
    void detailDoesNotExposeAnotherStudentsProgram() throws Exception {
        ...
        StudentProgramEntity foreignProgram =
                createProgram(foreign, "Чужая программа", Instant.now());

        mockMvc.perform(
                        get("/api/v1/student/programs/{studentProgramId}", foreignProgram.id())
    ...
    void topicReturnsOwnTopicWithProgressAndOrderedMaterials() throws Exception {
        ...
        LearningProgramEntity otherLearningProgram =
                learningProgramRepository.findById(otherProgram.learningProgramId()).orElseThrow();
        TopicEntity otherTopic =
                createTopic(createModule(otherLearningProgram, "Чужой модуль", 0), "Чужая тема", 0);
    ...
    void topicDoesNotExposeAnotherStudentsProgram() throws Exception {
        ...
        LearningProgramEntity learningProgram =
                learningProgramRepository
                        .findById(foreignProgram.learningProgramId())
                        .orElseThrow();
    ...
    void topicFromAnotherProgramIsNotFound() throws Exception {
        ...
        StudentProgramEntity requestedProgram =
                createProgram(fixture, "Первая программа", Instant.now());
        StudentProgramEntity otherProgram =
                createProgram(fixture, "Вторая программа", Instant.now());
        LearningProgramEntity otherLearningProgram =
                learningProgramRepository.findById(otherProgram.learningProgramId()).orElseThrow();
        TopicEntity otherTopic =
                createTopic(
                        createModule(otherLearningProgram, "Другой модуль", 0), "Другая тема", 0);
    ...
    void studentCannotDownloadFileFromAnotherStudentsProgram() throws Exception {
        ...
        LessonMaterialEntity material =
                createFileMaterial(foreign, topic, "foreign.txt", "secret".getBytes());
    ...
    private LessonMaterialEntity createMaterial(
            Fixture fixture, TopicEntity topic, String title, int position) {
    ...
    private LessonMaterialEntity createFileMaterial(
            Fixture fixture, TopicEntity topic, String filename, byte[] content) {
