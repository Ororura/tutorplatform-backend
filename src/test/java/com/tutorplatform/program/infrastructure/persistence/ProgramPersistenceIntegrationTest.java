...
class ProgramPersistenceIntegrationTest extends PostgresIntegrationTest {

    @DynamicPropertySource
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_program_persistence", "008");
    }
    ...
    void learningProgramBelongsToTeacher() {
        ...
        LearningProgramEntity persisted =
    ...
    void studentProgramLinksStudentAndLearningProgram() {
        ...
        StudentProgramEntity persisted =
                studentProgramRepository.findById(studentProgram.id()).orElseThrow();
    ...
    void studentProgramForAnotherStudentIsDifferent() {
        ...
        ProgramQuery.StudentProgramContext first =
                programQuery.findStudentProgram(firstProgram.id()).orElseThrow();
        ProgramQuery.StudentProgramContext second =
                programQuery.findStudentProgram(secondProgram.id()).orElseThrow();
    ...
    void topicLearningProgramIsResolvedThroughModule() {
        ...
        assertThat(
                        programQuery.topicBelongsToLearningProgram(
                                topic.id(), fixture.learningProgram().getId()))
                .isTrue();
        assertThat(
                        programQuery.topicBelongsToLearningProgram(
                                topic.id(), otherFixture.learningProgram().getId()))
                .isFalse();
    }
    ...
    void studentTopicProgressCompositePrimaryKeyWorks() {
        ...
        assertThat(studentTopicProgressRepository.findById(firstStudentProgram.id(), topic.id()))
                .get()
                .extracting(StudentTopicProgressEntity::status)
    ...
    void optimisticLockingWorksForAllVersionedEntities() {
        ...
        assertThat(learningProgramRepository.findById(fixture.learningProgram().getId()))
                .get()
                .extracting(LearningProgramEntity::getTitle)
        ...
        assertThat(studentProgramRepository.findById(studentProgram.id()))
                .get()
                .extracting(StudentProgramEntity::status)
    ...
    private StudentProgramEntity createStudentProgram(
            ProgramFixture fixture, StudentEntity student) {
    ...
    private StudentProgramEntity copyWithStatus(
            StudentProgramEntity source, StudentProgramStatus status) {
        return new StudentProgramEntity(
                source.id(),
                source.studentId(),
                source.learningProgramId(),
                source.assignedByTeacherId(),
                status,
                source.reportIntervalMinutes(),
                source.startedAt(),
                source.completedAt(),
                source.version(),
                source.createdAt(),
    ...
    private record ProgramFixture(
            TeacherEntity teacher, SubjectEntity subject, LearningProgramEntity learningProgram) {}
}
