...
class SessionApplicationIntegrationTest extends PostgresIntegrationTest {
    ...
    @Autowired private LearningPeriodService learningPeriodService;
    ...
    void rejectsStudentOwnedByAnotherTeacher() {
        ...
        assertThatThrownBy(
                        () ->
                                lessonSessionService.createLessonSession(
                                        owner.principal(), createCommand(foreign, List.of())))
                        ...
                        () ->
                                lessonSessionService.createLessonSession(
                                        ...
                                        createCommand(
                                                first,
                                                List.of(
                                                        new LessonSessionTopicInput(
                                                                second.firstTopic().id(), true)))))
                        ...
                        () ->
                                lessonSessionService.createLessonSession(
                                        ...
                                        createCommand(
                                                ...
                                                List.of(
                                                        new LessonSessionTopicInput(
                                                                target.firstTopic().id(), true),
                                                        new LessonSessionTopicInput(
                                                                otherProgram.firstTopic().id(),
                                                                false)))))
                .isInstanceOf(TopicOutsideStudentProgramException.class);
    ...
    void readsOwnedLessonSession() {
        ...
        LessonSessionResult found =
                lessonSessionService.getLessonSession(
        ...
        assertThat(found.topics())
                .extracting(LessonSessionTopicResult::topicId)
    ...
    void listsOnlyOwnedStudentSessionsWithDatabasePagination() {
        ...
        createSession(
                fixture,
                AttendanceStatus.MISSED,
                List.of(new LessonSessionTopicInput(fixture.firstTopic().id(), false)));
        createSession(fixture, AttendanceStatus.CANCELLED, List.of());
        createSession(other, AttendanceStatus.ATTENDED, List.of());

        LessonSessionPageResult firstPage =
                lessonSessionService.listLessonSessions(
                        fixture.principal(), fixture.student().getId(), 0, 2);
        LessonSessionPageResult secondPage =
                lessonSessionService.listLessonSessions(
        ...
        assertThat(firstPage.items())
                .allSatisfy(
                        session ->
                                assertThat(session.studentProgramId())
    ...
    void updatesAllowedLessonSessionFields() {
        ...
        LessonSessionResult updated =
                lessonSessionService.updateLessonSession(
                        fixture.principal(),
        ...
        LessonSessionResult updated =
                lessonSessionService.updateLessonSession(
                        ...
                        updateCommand(
                                ...
                                List.of(
                                        new LessonSessionTopicInput(
                                                fixture.secondTopic().id(), true))));

        assertThat(updated.topics())
                .extracting(LessonSessionTopicResult::topicId)
    ...
    void rejectsStaleUpdateWithOptimisticLockingAndKeepsTopics() {
        ...
        LessonSessionResult created =
                createSession(
                        ...
                        AttendanceStatus.ATTENDED,
                        List.of(new LessonSessionTopicInput(fixture.firstTopic().id(), true)));
        UpdateLessonSessionCommand firstUpdate =
                updateCommand(
                        created,
                        List.of(new LessonSessionTopicInput(fixture.secondTopic().id(), true)));
        ...
        LessonSessionResult persisted =
                lessonSessionService.getLessonSession(
                        fixture.principal(), fixture.student().getId(), created.id());
        assertThat(persisted.version()).isEqualTo(updated.version());
        assertThat(persisted.topics())
                .extracting(LessonSessionTopicResult::topicId)
                .containsExactly(fixture.secondTopic().id());
    }

    private LessonSessionResult createSession(
            SessionFixture fixture, AttendanceStatus status, List<LessonSessionTopicInput> topics) {
        return lessonSessionService.createLessonSession(
                fixture.principal(), createCommand(fixture, topics, status));
    }

    private CreateLessonSessionCommand createCommand(
            SessionFixture fixture, List<LessonSessionTopicInput> topics) {
        return createCommand(fixture, topics, AttendanceStatus.ATTENDED);
    }

    private CreateLessonSessionCommand createCommand(
            SessionFixture fixture, List<LessonSessionTopicInput> topics, AttendanceStatus status) {
    ...
    private UpdateLessonSessionCommand updateCommand(
            LessonSessionResult source, List<LessonSessionTopicInput> topics) {
    ...
    private SessionFixture createFixture(TeacherFixture teacherFixture, String firstName) {
        ...
        teacherStudentLinkRepository.saveAndFlush(
                new TeacherStudentLinkEntity(teacherFixture.teacher(), student));
        ...
        TopicEntity secondTopic =
                topicRepository.saveAndFlush(
                        new TopicEntity(
                                ...
                                1,
                                TopicStatus.DRAFT));
        return new SessionFixture(teacherFixture, student, studentProgram, firstTopic, secondTopic);
    }
