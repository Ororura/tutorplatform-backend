...
class SessionApiIntegrationTest extends PostgresIntegrationTest {

    @DynamicPropertySource
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_session_api", "008");
    }
    ...
    void postCreatesLessonSession() throws Exception {
        ...
        MvcResult result =
                mockMvc.perform(
                                post(sessionsUrl(fixture.student().getId()))
                        ...
                        .andExpect(
                                header().string(
                                                "Location",
                                                org.hamcrest.Matchers.containsString("/sessions/")))
                        .andExpect(
                        ...
                        .andExpect(
                                jsonPath("$.topics[0].topicId")
                                        .value(fixture.topic().id().toString()))
    ...
    void postCreatesEveryAttendanceStatus(AttendanceStatus statusValue) throws Exception {
        SessionFixture fixture =
                createFixture("api-status-" + statusValue.name().toLowerCase() + "@example.com");
    ...
    void getListsLessonSessionsWithPaginationAndSort() throws Exception {
        ...
        mockMvc.perform(
                        get(sessionsUrl(fixture.student().getId()))
                                .with(user(fixture.principal()))
                                .param("page", "0")
                                .param("size", "1")
    ...
    void getReturnsLessonSessionDetails() throws Exception {
        ...
        mockMvc.perform(
                        get(sessionUrl(fixture.student().getId(), session.id()))
                                .with(user(fixture.principal())))
                .andExpect(status().isOk())
    ...
    void patchUpdatesLessonSession() throws Exception {
        ...
        mockMvc.perform(
                        patch(sessionUrl(fixture.student().getId(), session.id()))
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void stalePatchReturnsLessonSessionVersionConflict() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Lesson session was modified by another request"));
    }
    ...
    void unknownLessonSessionReturnsNotFoundContract() throws Exception {
        ...
        mockMvc.perform(
                        get(sessionUrl(fixture.student().getId(), UUID.randomUUID()))
                                .with(user(fixture.principal())))
    ...
    void foreignStudentIsNormalizedToNotFound() throws Exception {
        ...
        mockMvc.perform(get(sessionsUrl(owner.student().getId())).with(user(other.principal())))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("STUDENT_NOT_FOUND"));

        mockMvc.perform(
                        post(sessionsUrl(owner.student().getId()))
                                .with(user(other.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        createRequest(
                                                owner.studentProgram().id(),
                                                owner.topic().id(),
                                                60)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("STUDENT_NOT_FOUND"));
    ...
    void invalidDurationReturnsValidationError(int durationMinutes) throws Exception {
        SessionFixture fixture =
                createFixture("api-invalid-duration-" + durationMinutes + "@example.com");
    ...
    void foreignLessonSessionIsNotReadableOrUpdatable() throws Exception {
        ...
        mockMvc.perform(
                        get(sessionUrl(owner.student().getId(), session.id()))
                                .with(user(other.principal())))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("STUDENT_NOT_FOUND"));

        mockMvc.perform(
                        patch(sessionUrl(owner.student().getId(), session.id()))
                                .with(user(other.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void invalidStudentProgramReturnsNotFoundContract() throws Exception {
        ...
        mockMvc.perform(
                        post(sessionsUrl(fixture.student().getId()))
                                ...
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        createRequest(UUID.randomUUID(), fixture.topic().id(), 60)))
                .andExpect(status().isNotFound())
    ...
    void openApiPublishesSessionOperationsAndSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/sessions'].post.operationId")
                                .value("createLessonSession"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/sessions'].get.operationId")
                                .value("listLessonSessions"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/sessions/{sessionId}'].get.operationId")
                                .value("getLessonSession"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/sessions/{sessionId}'].patch.operationId")
                                .value("updateLessonSession"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.CreateLessonSessionRequest.properties.studentProgramId.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.CreateLessonSessionRequest.properties.startedAt.format")
                                .value("date-time"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.CreateLessonSessionRequest.properties.attendanceStatus.enum.length()")
                                .value(3))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.LessonSessionDetailsResponse.properties.id.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.LessonSessionDetailsResponse.properties.startedAt.format")
                                .value("date-time"));
    }

    private LessonSessionResult createSession(
            SessionFixture fixture, int durationMinutes, AttendanceStatus status) {
