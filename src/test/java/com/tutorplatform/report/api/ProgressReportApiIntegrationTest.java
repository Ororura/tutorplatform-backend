...
class ProgressReportApiIntegrationTest extends PostgresIntegrationTest {

    @DynamicPropertySource
    static void configurePostgres(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_progress_report_api", "008");
    }
                    ...
                    """
                        ...
                        .andExpect(
                                jsonPath("$.studentProgramId")
                                        .value(fixture.studentProgramId().toString()))
    ...
    @Test
    void createRejectsActiveDuplicateForeignAndMismatchedPeriodsWithoutPartialRows()
            throws Exception {
        ...
        performCreate(owner, owner.studentProgramId(), active).andExpect(status().isConflict());
        jdbc.update("delete from learning_periods where id = ?", active);

        UUID completed = period(owner, "COMPLETED", owner.studentProgramId());
        performCreate(owner, owner.studentProgramId(), completed).andExpect(status().isCreated());
    ...
    void detailReturnsPersistedHistoricalSnapshotAndHidesForeignReport() throws Exception {
        ...
        jdbc.update(
                "insert into lesson_sessions(id, student_program_id, teacher_id, started_at, duration_minutes, attendance_status) values (?, ?, ?, ?, 500, 'ATTENDED')",
                UUID.randomUUID(),
                owner.studentProgramId(),
                owner.teacherId(),
                Timestamp.from(END));
    ...
    void teacherDownloadsOnlyOwnedPublishedHistoricalPdfWithoutCsrf() throws Exception {
        ...
        UUID published =
                report(owner, "PUBLISHED", START, "Русский комментарий\nSecond line", "План");
        ...
        MvcResult first =
                mockMvc.perform(get(url).with(user(owner.teacherPrincipal())))
                        .andExpect(status().isOk())
        ...
        jdbc.update(
                "insert into lesson_sessions(id, student_program_id, teacher_id, started_at, duration_minutes, attendance_status, private_notes) values (?, ?, ?, ?, 500, 'ATTENDED', 'DO_NOT_LEAK_PRIVATE_NOTES')",
                UUID.randomUUID(),
                owner.studentProgramId(),
                owner.teacherId(),
                Timestamp.from(END));
        ...
        assertThat(pdfText(second))
                .isEqualTo(firstText)
                .doesNotContain("DO_NOT_LEAK_PRIVATE_NOTES");

        mockMvc.perform(
                        get(REPORTS_URL + "/" + draft + "/pdf")
                                .with(user(owner.teacherPrincipal())))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("PROGRESS_REPORT_PDF_NOT_AVAILABLE"));
        mockMvc.perform(
                        get(REPORTS_URL + "/" + archived + "/pdf")
                                .with(user(owner.teacherPrincipal())))
                .andExpect(status().isConflict());
    ...
    void patchHasTruePatchSemanticsPreservesSnapshotAndRejectsStaleOrPublishedChanges()
        ...
        mockMvc.perform(get(REPORTS_URL + "/" + reportId).with(user(fixture.teacherPrincipal())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.teacherSummary").value((Object) null))
                .andExpect(jsonPath("$.nextPeriodPlan").value("Keep"))
                .andExpect(jsonPath("$.version").value(1));
    ...
    void securityCsrfAndOpenApiContractAreExposed() throws Exception {
        ...
        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/reports'].post.operationId")
                                .value("createProgressReport"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/reports'].get.operationId")
                                .value("listProgressReports"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/reports/{reportId}'].get.operationId")
                                .value("getProgressReport"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/reports/{reportId}'].patch.operationId")
                                .value("updateProgressReport"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/reports/{reportId}/pdf'].get.operationId")
                                .value("downloadProgressReportPdf"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/reports/{reportId}/pdf'].get.responses['200'].content['application/pdf'].schema.format")
                                .value("binary"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/reports/{reportId}/publish'].post.operationId")
                                .value("publishProgressReport"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.ProgressReportDetailsResponse.properties.snapshotSchemaVersion")
                                .exists())
                .andExpect(jsonPath("$.components.schemas.ProgressReportSnapshotV1").exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.ProgressReportSummaryResponse.properties.snapshot")
                                .doesNotExist())
    ...
    private org.springframework.test.web.servlet.ResultActions performCreate(
            Fixture fixture, UUID studentProgramId, UUID learningPeriodId) throws Exception {
    ...
    private UUID period(Fixture fixture, String status, UUID studentProgramId) {
        ...
        jdbc.update(
            ...
            """,
                ...
                completed ? 10 : null,
                status,
                completed ? Timestamp.from(START) : null,
                completed ? Timestamp.from(END) : null);
        return id;
    }

    private UUID report(
            Fixture fixture, String status, Instant createdAt, String summary, String plan) {
        UUID id = UUID.randomUUID();
