...
@Transactional
class ReportShareApiIntegrationTest extends PostgresIntegrationTest {
    ...
    static void configure(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(registry, "test_report_share_api", "008");
    ...
    void createsPublishedReportShareWithHashedOneTimeTokenAndValidatesInput() throws Exception {
        ...
        MvcResult result =
                create(owner, published, expiresAt)
    ...
    void listsOnlyOwnedMetadataWithDerivedStatuses() throws Exception {
        ...
        UUID expired =
                share(owner, reportId, "expired-token", Instant.now().minusSeconds(10), null);
        UUID revoked = share(owner, reportId, "revoked-token", null, Instant.now());
        share(foreign, foreignReport, "foreign-token", null, null);

        MvcResult result =
                mockMvc.perform(get(sharesUrl(reportId)).with(user(owner.principal())))
    ...
    void revokeIsIdempotentPersistentOwnershipScopedAndDisablesPublicAccess() throws Exception {
        ...
        assertThat(
                        jdbc.queryForObject(
                                "select revoked_at from report_shares where id = ?",
                                Instant.class,
                                shareId))
                .isNotNull();
        revoke(owner, reportId, shareId).andExpect(status().isNoContent());
        assertThat(
                        jdbc.queryForObject(
                                "select count(*) from report_shares where id = ?",
                                Integer.class,
                                shareId))
                .isOne();
    ...
    void publicReadUsesPersistedSnapshotAndReturnsNoInternalOrSensitiveData() throws Exception {
        ...
        JsonNode created =
                json(create(fixture, reportId, null).andExpect(status().isCreated()).andReturn());
        String rawToken = tokenFrom(created);

        MvcResult first =
                mockMvc.perform(get("/api/v1/public/reports/{token}", rawToken))
                        .andExpect(status().isOk())
                        ...
                        .andExpect(
                                jsonPath("$.snapshot.topics.completed[0].title")
        ...
        jdbc.update(
                "insert into lesson_sessions(id, student_program_id, teacher_id, started_at, duration_minutes, attendance_status, private_notes) values (?, ?, ?, now(), 120, 'ATTENDED', 'privateNotes-leak@example.com')",
                sessionId,
                fixture.studentProgramId(),
                fixture.teacherId());
        jdbc.update(
                "insert into teacher_assessments(id, lesson_session_id, understanding_score, public_comment) values (?, ?, 5, 'changed assessment')",
                UUID.randomUUID(),
                sessionId);
        jdbc.update(
                "update topics set title = 'Renamed source topic' where id = ?", fixture.topicId());
        ...
        assertThat(second.getResponse().getContentAsString())
                ...
                .doesNotContain(
                        ...
                        fixture.teacherId().toString(),
                        fixture.studentId().toString(),
                        fixture.studentProgramId().toString(),
                        fixture.topicId().toString(),
                        rawToken,
                        "tokenHash",
                        "privateNotes",
                        "@example.com",
    ...
    void publicLookupUsesEstablishedNotFoundGoneAndPublishedOnlySemantics() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/public/reports/revoked-token"))
                .andExpect(status().isGone())
                .andExpect(jsonPath("$.code").value("REPORT_SHARE_REVOKED"));
        mockMvc.perform(get("/api/v1/public/reports/draft-token")).andExpect(status().isNotFound());
    }
    ...
    void publicPdfUsesTheSameShareValidationAndLeaksNoPrivateSourceData() throws Exception {
        ...
        jdbc.update(
                "update users set email = 'do-not-leak@example.com' where id = ?",
                fixture.userId());
        jdbc.update(
                "insert into lesson_sessions(id, student_program_id, teacher_id, started_at, duration_minutes, attendance_status, private_notes) values (?, ?, ?, now(), 120, 'ATTENDED', ?)",
                UUID.randomUUID(),
                fixture.studentProgramId(),
                fixture.teacherId(),
        ...
        jdbc.update(
                "insert into tasks(id, teacher_id, subject_id, title, description_markdown, task_type, status) values (?, ?, ?, 'Private task', '', 'CODE', 'ACTIVE')",
                taskId,
                fixture.teacherId(),
                fixture.subjectId());
        jdbc.update(
                "insert into task_test_cases(id, task_id, expected_output, hidden, position) values (?, ?, 'DO_NOT_LEAK_HIDDEN_TEST', true, 0)",
                UUID.randomUUID(),
                taskId);
        jdbc.update(
                "insert into submissions(id, student_id, student_program_id, task_id, attempt_no, status, text_answer) values (?, ?, ?, ?, 1, 'PASSED', 'DO_NOT_LEAK_TEXT_ANSWER')",
                submissionId,
                fixture.studentId(),
                fixture.studentProgramId(),
                taskId);
        jdbc.update(
                "insert into code_submissions(submission_id, source_code, execution_status) values (?, 'DO_NOT_LEAK_SOURCE', 'PASSED')",
                submissionId);
        ...
        MvcResult first =
                mockMvc.perform(get(url))
                        ...
                        .andExpect(
                                header().string(
                                                "Content-Disposition",
                                                "attachment; filename=\"progress-report-"
                                                        + published
                                                        + ".pdf\""))
                        .andReturn();
        ...
        assertThat(firstText)
                ...
                .doesNotContain(
                        "DO_NOT_LEAK_PRIVATE_NOTES",
                        "DO_NOT_LEAK_TEXT_ANSWER",
                        "DO_NOT_LEAK_SOURCE",
                        "DO_NOT_LEAK_HIDDEN_TEST",
        ...
        jdbc.update(
                "update topics set title = 'Changed after publication' where id = ?",
                fixture.topicId());
        MvcResult second = mockMvc.perform(get(url)).andExpect(status().isOk()).andReturn();
        assertThat(pdfText(second))
                .isEqualTo(firstText)
                .doesNotContain("Changed after publication");
    ...
    void teacherSecurityCsrfAndOpenApiContractAreExact() throws Exception {
        ...
        mockMvc.perform(
                        post(sharesUrl(reportId))
                                .with(user(fixture.principal()))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{}"))
                .andExpect(status().isForbidden());
        mockMvc.perform(delete(sharesUrl(reportId) + "/" + shareId).with(user(fixture.principal())))
                .andExpect(status().isForbidden());
        mockMvc.perform(get("/api/v1/public/reports/security-token")).andExpect(status().isOk());

        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/reports/{reportId}/shares'].post.operationId")
                                .value("createReportShare"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/reports/{reportId}/shares'].get.operationId")
                                .value("listReportShares"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/reports/{reportId}/shares/{shareId}'].delete.operationId")
                                .value("revokeReportShare"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/public/reports/{token}'].get.operationId")
                                .value("getPublicProgressReport"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/public/reports/{token}/pdf'].get.operationId")
                                .value("downloadPublicProgressReportPdf"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/public/reports/{token}/pdf'].get.responses['200'].content['application/pdf'].schema.format")
                                .value("binary"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/public/reports/{token}/pdf'].get.security")
                                .isEmpty())
                .andExpect(
                        jsonPath("$.paths['/api/v1/public/reports/{token}'].get.security")
                                .isEmpty())
                ...
                .andExpect(
                        jsonPath("$.components.schemas.ReportShareStatus.enum.length()").value(3))
    ...
    private org.springframework.test.web.servlet.ResultActions create(
            Fixture fixture, UUID reportId, Instant expiresAt) throws Exception {
    ...
    private org.springframework.test.web.servlet.ResultActions revoke(
            Fixture fixture, UUID reportId, UUID shareId) throws Exception {
        return mockMvc.perform(
                delete(sharesUrl(reportId) + "/" + shareId)
                        .with(user(fixture.principal()))
                        .with(csrf()));
    }
    ...
    private Fixture fixture(String suffix) {
        ...
        jdbc.update(
                ...
                userId);
        jdbc.update(
                "insert into students(id, first_name, last_name) values (?, 'Student', 'PrivateSurname')",
                studentId);
        jdbc.update(
        ...
        jdbc.update(
                ...
                programId);
        jdbc.update(
                "insert into topics(id, module_id, title, position, status) values (?, ?, 'Historical title', 0, 'ACTIVE')",
                topicId,
                moduleId);
    ...
    private UUID report(Fixture fixture, String status) {
        ...
        Instant publishedAt =
                status.equals("PUBLISHED") ? Instant.parse("2026-02-01T10:00:00Z") : null;
        String snapshot =
                """
        ...
        jdbc.update(
            ...
            """,
                ...
                fixture.teacherId(),
                status,
                Timestamp.from(PERIOD_START),
                Timestamp.from(PERIOD_END),
                snapshot,
