...
class HttpExecutionAdapterTest {
    ...
    void sendsInternalWorkerContractWithLanguageModesLimitsAndBoundedOutput() throws Exception {
        ...
        assertThat(json.path("testCases").get(0).path("comparisonMode").asText()).isEqualTo("EXACT");
        assertThat(json.path("testCases").get(1).path("comparisonMode").asText()).isEqualTo("NORMALIZED");
    ...
    void mapsEveryWorkerStatus(ExecutionStatus status) throws Exception {
        ...
        server.createContext("/internal/v1/executions", exchange ->
            respond(exchange, 200, response(request, status.name(), "out", null))
        );
        server.start();
        ...
        assertThat(result.testResults()).singleElement().satisfies(testResult -> {
            assertThat(testResult.testCaseId()).isEqualTo(request.testCases().getFirst().id());
    ...
    void connectionTimeoutMapsToSystemError() {
        ...
        var result = adapter(
            URI.create("http://192.0.2.1:81"), Duration.ofMillis(25), Duration.ofMillis(25), 64
        ).execute(request);
    ...
    void readTimeoutMapsToSystemError() {
        var request = new ExecutionRequest(
            UUID.randomUUID(), ExecutionLanguage.PYTHON, "print(input())", 100, 128,
        ...
        var result = adapter(baseUrl, Duration.ofMillis(100), Duration.ofMillis(25), 64).execute(request);
    ...
    void malformedResponseNeverMapsToSolutionStatus() {
        var request = request(List.of(testCase(ExecutionComparisonMode.EXACT, "expected")));
        server.createContext("/internal/v1/executions", exchange -> respond(exchange, 200, "{\"status\":\"PASSED\"}"));
        server.start();
    ...
    void logsCorrelationAndStatusWithoutSourceOrHiddenExpectedOutput() {
        var request = request(List.of(testCase(ExecutionComparisonMode.EXACT, "do-not-log-hidden")));
        server.createContext("/internal/v1/executions", exchange ->
            respond(exchange, 200, response(request, "PASSED", null, null))
        );
        server.start();
        ...
        assertThat(messages).anyMatch(message -> message.contains(request.executionId().toString()));
    ...
    private HttpExecutionAdapter adapter(URI uri, Duration connectTimeout, Duration readTimeout, int maxBytes) {
    ...
    private static ExecutionRequest request(List<ExecutionTestCase> testCases) {
        return new ExecutionRequest(
            UUID.randomUUID(), ExecutionLanguage.PYTHON, "print(input())", 5_000, 128, testCases
        );
    }
    ...
    private static String response(ExecutionRequest request, String status, String stdout, String stderr) {
        return """
            {
              "executionId": "%s",
              "status": "%s",
