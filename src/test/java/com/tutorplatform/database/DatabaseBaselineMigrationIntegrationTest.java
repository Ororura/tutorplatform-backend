...
class DatabaseBaselineMigrationIntegrationTest extends PostgresIntegrationTest {
    ...
    @Test
    void cleanDatabaseMigratesThroughV010() throws SQLException {
        ...
        assertThat(queryStrings("""
            ...
            .containsExactlyInAnyOrder(
                ...
                "students", "teacher_student_links", "student_invites",
                "subjects", "learning_programs", "student_programs", "modules", "topics",
                "student_topic_progress", "file_assets", "lesson_materials",
    ...
    void performanceIndexesMatchErModelAndRemainNonUnique() throws SQLException {
        ...
        Map<String, ExpectedIndex> expected = Map.ofEntries(
            expected("ix_student_program_student_status", "student_programs", "(student_id, status)"),
            expected("ix_topic_progress_program_status", "student_topic_progress",
                "(student_program_id, status)"),
            expected("ix_session_program_started", "lesson_sessions",
                "(student_program_id, started_at DESC)"),
            expected("ix_session_teacher_started", "lesson_sessions", "(teacher_id, started_at DESC)"),
            expected("ix_task_teacher_subject_status", "tasks", "(teacher_id, subject_id, status)"),
            expected("ix_homework_program_assigned", "homeworks",
                "(student_program_id, assigned_at DESC)"),
            expected("ix_homework_program_due_active", "homeworks", "(student_program_id, due_at)",
                "status", "'ASSIGNED'"),
            expected("ix_submission_program_submitted", "submissions",
                "(student_program_id, submitted_at DESC)"),
            expected("ix_submission_student_task_submitted", "submissions",
                "(student_id, task_id, submitted_at DESC)"),
            expected("ix_submission_homework_item_submitted", "submissions",
                "(homework_item_id, submitted_at DESC)", "homework_item_id IS NOT NULL"),
            expected("ix_report_program_created", "progress_reports",
                "(student_program_id, created_at DESC)")
        );

        expected.forEach((name, expectedIndex) -> {
            ...
            if (expectedIndex.predicateFragments().isEmpty()) {
                ...
                assertThat(index.predicate()).as(name).contains(expectedIndex.predicateFragments());
            }
        });
    }
    ...
    void importantReadQueryShapesAreExplainable() throws SQLException {
        ...
        List<String> queries = List.of(
            "select * from lesson_sessions where student_program_id = '%s' order by started_at desc".formatted(id),
            "select * from homeworks where student_program_id = '%s' order by assigned_at desc".formatted(id),
            "select * from homeworks where student_program_id = '%s' and status = 'ASSIGNED' order by due_at"
                .formatted(id),
            "select * from submissions where student_program_id = '%s' order by submitted_at desc".formatted(id),
            "select * from submissions where student_id = '%1$s' and task_id = '%1$s' order by submitted_at desc"
                .formatted(id),
            "select * from progress_reports where student_program_id = '%s' order by created_at desc".formatted(id)
        );
    ...
    private static Map<String, IndexMetadata> readIndexMetadata() throws SQLException {
        ...
        try (Connection connection = connection();
             ...
             var rows = statement.executeQuery()) {
            while (rows.next()) {
                indexes.put(rows.getString("index_name"), new IndexMetadata(
    ...
    private record ExpectedIndex(String tableName, String columns, List<String> predicateFragments) {
    }

    private record IndexMetadata(String tableName, boolean unique, String definition, String predicate) {
    }
}
