...
class MaterialPersistenceIntegrationTest {
    ...
    void linkMaterialRequiresExternalUrl() {
        ...
        assertThat(lessonMaterialRepository.findById(link.getId()))
            .get().extracting(LessonMaterialEntity::getExternalUrl)
    ...
    void lessonMaterialOptimisticLockingWorks() {
        ...
        LessonMaterialEntity first = transaction.execute(status -> lessonMaterialRepository
            .findById(material.getId()).orElseThrow());
        LessonMaterialEntity stale = transaction.execute(status -> lessonMaterialRepository
            .findById(material.getId()).orElseThrow());
        ...
        Throwable thrown = catchThrowable(() -> transaction.executeWithoutResult(status -> {
            stale.update(LessonMaterialType.TEXT, "Устаревшая версия", "Текст", null, null, 0);
        ...
        assertThat(lessonMaterialRepository.findById(material.getId()))
            .get().extracting(LessonMaterialEntity::getTitle)
    ...
    private void insertMaterialWithoutRequiredValue(ContentFixture fixture, String type, int position) {
        jdbcTemplate.update(
            """
                insert into lesson_materials (
