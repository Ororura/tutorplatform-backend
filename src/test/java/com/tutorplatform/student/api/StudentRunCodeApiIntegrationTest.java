...
class StudentRunCodeApiIntegrationTest extends PostgresIntegrationTest {
                            ...
                            return new ExecutionResult(
                                    ...
                                    List.of(
                                            new ExecutionTestResult(
                                                    fixture.publicTestId(), true, 20, "5", null),
                                            new ExecutionTestResult(
                                                    fixture.hiddenTestId(),
                                                    false,
                                                    22,
                                                    "secret answer",
                                                    "secret")));
    ...
    void openApiPublishesOnlyPublicRunSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.paths['/api/v1/student/tasks/{taskId}/run'].post.operationId")
                                .value("runCode"))
                .andExpect(
                        jsonPath("$.components.schemas.RunCodeRequest.properties.sourceCode")
                                .exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.RunCodeRequest.properties.homeworkItemId.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath("$.components.schemas.RunCodeRequest.properties.language")
                                .doesNotExist())
                .andExpect(jsonPath("$.components.schemas.RunCodeResponse").exists())
                .andExpect(
                        jsonPath("$.components.schemas.StudentRunCodeTestResultResponse").exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentRunCodeTestResultResponse.properties.input")
                                .doesNotExist())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentRunCodeTestResultResponse.properties.expectedOutput")
                                .doesNotExist())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentRunCodeTestResultResponse.properties.actualOutput")
                                .doesNotExist())
                .andExpect(
                        jsonPath("$.components.schemas.TaskTestCaseResponse.properties.inputText")
                                .exists())
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.TaskTestCaseResponse.properties.expectedOutput")
                                .exists())
    ...
    private org.springframework.test.web.servlet.ResultActions performRun(
            AuthenticatedUser principal, UUID taskId, UUID homeworkItemId) throws Exception {
        return mockMvc.perform(
                post("/api/v1/student/tasks/{taskId}/run", taskId)
                        .with(user(principal))
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody(homeworkItemId)));
    }

    private String requestBody(UUID homeworkItemId) throws Exception {
        return objectMapper.writeValueAsString(
                Map.of("sourceCode", "pass", "homeworkItemId", homeworkItemId));
    }
    ...
    private Fixture createFixture(
            String label, TaskType taskType, TaskStatus taskStatus, boolean executionEnabled) {
