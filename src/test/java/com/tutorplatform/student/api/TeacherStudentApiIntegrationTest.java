...
class TeacherStudentApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void listReturnsOnlyCurrentTeachersStudents() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/teacher/students").with(user(firstTeacher.principal())))
    ...
    void detailReturnsAccountAndPrimaryRelation() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/teacher/students/{studentId}", student.getId())
                                .with(user(teacher.principal())))
                .andExpect(status().isOk())
    ...
    void updateChangesOnlyProvidedNameFields() throws Exception {
        ...
        mockMvc.perform(
                        patch("/api/v1/teacher/students/{studentId}", student.getId())
                                .with(user(teacher.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
    ...
    void openApiPublishesStableTeacherStudentOperationIds() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/students'].get.operationId")
                                .value("listTeacherStudents"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students'].get.parameters[0].schema.type")
                                .value("integer"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students'].get.parameters[1].schema.type")
                                .value("integer"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/students'].post.operationId")
                                .value("createStudent"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/students/{studentId}'].get.operationId")
                                .value("getStudent"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}'].patch.operationId")
                                .value("updateStudent"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}'].get.parameters[0].schema.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath("$.components.schemas.StudentSummaryResponse.properties.id.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentSummaryResponse.properties.createdAt.format")
                                .value("date-time"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentSummaryResponse.properties.status.enum.length()")
                                .value(3))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentSummaryResponse.properties.accountStatus.enum.length()")
                                .value(3));
    }
    ...
    private StudentEntity createStudent(TeacherEntity teacher, String firstName, String lastName) {
        StudentEntity student =
                studentRepository.saveAndFlush(
                        new StudentEntity(
                                UUID.randomUUID(), firstName, lastName, StudentStatus.ACTIVE));
