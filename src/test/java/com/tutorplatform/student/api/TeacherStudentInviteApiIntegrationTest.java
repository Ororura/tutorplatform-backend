...
class TeacherStudentInviteApiIntegrationTest extends PostgresIntegrationTest {
    ...
    static void configureApplication(DynamicPropertyRegistry registry) {
        PostgresIntegrationTest.configurePostgres(
                registry, "test_teacher_student_invite_api", null);
    ...
    @Test
    void createReturnsRawTokenOnceButStoresOnlyHashAndDoesNotLogToken(CapturedOutput output)
            throws Exception {
        ...
        MvcResult result =
                createInvite(teacher, student.getId(), "  student@example.com  ")
        ...
        assertThat(persisted.getExpiresAt())
                .isBetween(before.plus(2, ChronoUnit.DAYS), Instant.now().plus(2, ChronoUnit.DAYS));
        assertThat(output.getAll()).doesNotContain(rawToken);
    }

    @Test
    void creatingAnotherInviteRevokesPreviousActiveInviteAndListNeverReturnsSecrets()
            throws Exception {
        TeacherContext teacher = createTeacher("teacher@example.com");
        StudentEntity student = createStudent(teacher.teacher());
        ...
        JsonNode first =
                json(
                        createInvite(teacher, student.getId(), "first@example.com")
                                .andExpect(status().isCreated())
                                .andReturn());
        String firstRawToken = tokenFrom(first);
        JsonNode second =
                json(
                        createInvite(teacher, student.getId(), "second@example.com")
                                .andExpect(status().isCreated())
                                .andReturn());
        ...
        MvcResult listResult =
                mockMvc.perform(
                                get("/api/v1/teacher/students/{studentId}/invites", student.getId())
    ...
    void createChecksOwnershipRegistrationStateAndUserEmailConflicts() throws Exception {
        ...
        jdbcTemplate.update(
                "update students set user_id = ? where id = ?", studentUser.id(), student.getId());
    ...
    void revokeIsIdempotentAndRejectsAcceptedOrInvisibleInvites() throws Exception {
        ...
        UUID inviteId =
                UUID.fromString(
                        json(createInvite(owner, student.getId(), "student@example.com")
                                        .andExpect(status().isCreated())
                                        .andReturn())
                                .required("id")
                                .textValue());
        ...
        revoke(owner, student.getId(), inviteId).andExpect(status().isNoContent());
        assertThat(studentInviteRepository.findById(inviteId).orElseThrow().getRevokedAt())
                .isNotNull();
    ...
    void openApiPublishesInviteOperationIdsAndStatusEnum() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/invites'].post.operationId")
                                .value("createStudentInvite"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/invites'].get.operationId")
                                .value("listStudentInvites"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/invites/{inviteId}'].delete.operationId")
                                .value("revokeStudentInvite"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentInviteCreatedResponse.properties.id.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentInviteCreatedResponse.properties.studentId.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentInviteCreatedResponse.properties.expiresAt.format")
                                .value("date-time"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentInviteSummaryResponse.properties.createdAt.format")
                                .value("date-time"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.StudentInviteSummaryResponse.properties.status.enum.length()")
                                .value(4))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/students/{studentId}/invites/{inviteId}'].delete.responses['409'].content['application/json'].schema['$ref']")
                                .value("#/components/schemas/ApiError"));
    }

    private org.springframework.test.web.servlet.ResultActions createInvite(
            TeacherContext teacher, UUID studentId, String email) throws Exception {
        return mockMvc.perform(
                post("/api/v1/teacher/students/{studentId}/invites", studentId)
                        .with(user(teacher.principal()))
                        .with(csrf())
    ...
    private org.springframework.test.web.servlet.ResultActions revoke(
            TeacherContext teacher, UUID studentId, UUID inviteId) throws Exception {
        return mockMvc.perform(
                delete(
    ...
    private StudentInviteEntity persistInvite(
            TeacherEntity teacher, StudentEntity student, String email, Instant expiresAt) {
