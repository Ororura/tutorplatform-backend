...
class StudentRunCodeServiceTest {
    ...
    void mapsOnlyServerTaskConfigurationAndGeneratesExecutionId() {
        ...
        RunCodeResult result =
                service.run(principal, taskId, homeworkItemId, null, null, "print(input())");
        ...
        assertThat(request.testCases())
                .extracting(ExecutionTestCase::id)
                .containsExactly(publicTest.id(), hiddenTest.id());
        assertThat(request.testCases())
                .extracting(ExecutionTestCase::comparisonMode)
        ...
        assertThat(result.tests().get(1)).isEqualTo(new RunCodeResult.TestResult(1, true, false));
                ...
                .thenAnswer(
                        invocation -> {
                            ...
                            return new ExecutionResult(
                                    request.executionId(),
                                    status,
                                    status == ExecutionStatus.PASSED ? 2 : 0,
                                    2,
                                    10,
                                    null,
                                    null,
    ...
    void unavailableWorkerProducesSanitizedSystemError() {
        when(executionPort.execute(any()))
                .thenThrow(new IllegalStateException("docker secret path"));
    ...
    void foreignStudentProgramIsRejectedBeforeExecution() {
        when(programQuery.findStudentProgram(studentProgramId))
                .thenReturn(
                        Optional.of(
    ...
    void taskOutsideHomeworkItemIsRejectedBeforeExecution() {
        when(homeworkQuery.findStudentTaskContext(homeworkItemId))
                .thenReturn(
                        Optional.of(
    ...
    void textAndInactiveTasksAreNotExecutable() {
        when(taskQuery.findCodeTaskConfiguration(taskId))
                .thenReturn(Optional.of(task(TaskType.TEXT, TaskStatus.ACTIVE, true)));
        assertReason(Reason.TASK_NOT_EXECUTABLE);

        when(taskQuery.findCodeTaskConfiguration(taskId))
                .thenReturn(Optional.of(task(TaskType.CODE, TaskStatus.ARCHIVED, true)));
        assertReason(Reason.TASK_NOT_EXECUTABLE);
    }
    ...
    void executionDisabledIsControlled() {
        when(taskQuery.findCodeTaskConfiguration(taskId))
                .thenReturn(Optional.of(task(TaskType.CODE, TaskStatus.ACTIVE, false)));

        assertReason(Reason.TASK_EXECUTION_DISABLED);
    }

    private void assertReason(Reason reason) {
        assertThatThrownBy(() -> service.run(principal, taskId, homeworkItemId, null, null, "pass"))
                .isInstanceOfSatisfying(
                        RunCodeException.class,
                        exception -> assertThat(exception.reason()).isEqualTo(reason));
        verifyNoInteractions(executionPort);
    }

    private void allowValidContext() {
        when(studentOwnershipQuery.findStudentIdByUserId(userId))
                .thenReturn(Optional.of(studentId));
        when(taskQuery.findTask(taskId))
                .thenReturn(
                        Optional.of(
                                new TaskQuery.TaskContext(
                                        taskId,
                                        teacherId,
                                        subjectId,
                                        "Task",
                                        ...
                                        TaskStatus.ACTIVE)));
        when(taskQuery.findCodeTaskConfiguration(taskId))
                .thenReturn(Optional.of(task(TaskType.CODE, TaskStatus.ACTIVE, true)));
        when(homeworkQuery.findStudentTaskContext(homeworkItemId))
                .thenReturn(
                        Optional.of(
                                new HomeworkQuery.StudentTaskContext(
    ...
    private TaskQuery.CodeTaskConfiguration task(
            TaskType type, TaskStatus status, boolean executionEnabled) {
