...
class TaskApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void teacherCreatesTextTaskWithServerDefaults() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        header().string(
                                        "Location",
                                        org.hamcrest.Matchers.containsString("/teacher/tasks/")))
    ...
    void missingOrForeignSubjectIsNotDisclosed() throws Exception {
        ...
        mockMvc.perform(
                        post(tasksUrl())
                                ...
                                .content(
                                        createRequest(
                                                UUID.randomUUID(), "Missing", "Ответ", "EASY")))
                                        ...
                                        new CreateTaskCommand(
                                                ...
                                                List.of(
                                                        new TaskTestCaseInput(
                                                                ...
                                                                0)))))
                .isInstanceOf(
                        com.tutorplatform.task.application.exception
                                .InvalidProgrammingTaskConfigException.class);
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () ->
                                taskService.createTask(
                                        fixture.principal(),
                                        new CreateTaskCommand(
                                                ...
                                                List.of(
                                                        new TaskTestCaseInput(
                                                                ...
                                                                0)))))
                .isInstanceOf(
                        com.tutorplatform.task.application.exception.InvalidTaskTestCaseException
                                .class);
    ...
    void codeConfigurationAcceptsBoundaryLimitsAndRejectsOutsideValues() throws Exception {
        ...
        mockMvc.perform(
                        put(taskUrl(taskId) + "/programming-config")
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
        ...
        for (String body :
                List.of(
                        configUpdateRequest(99, 128),
                        configUpdateRequest(30001, 128),
                        configUpdateRequest(5000, 15),
                        configUpdateRequest(5000, 1025))) {
            mockMvc.perform(
                            put(taskUrl(taskId) + "/programming-config")
                                    .with(user(fixture.principal()))
                                    .with(csrf())
    ...
    void bulkReplacePreservesValidItemsAndIsAtomicOnDuplicatePosition() throws Exception {
        ...
        assertThat(testCaseRepository.findAllByTaskId(taskId))
                .extracting(TaskTestCase::expectedOutput)
                .containsExactly("7", "0");
    }
    ...
    void negativePositionNullOutputAndTextProgrammingOperationsAreRejected() throws Exception {
        ...
        mockMvc.perform(
                        put(taskUrl(text.id()) + "/programming-config")
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(configUpdateRequest(5000, 128)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("TASK_TYPE_MISMATCH"));
        mockMvc.perform(
                        put(taskUrl(text.id()) + "/test-cases")
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(validSingleTestRequest()))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("TASK_TYPE_MISMATCH"));
    }
    ...
    void taskAttachesToOwnedTopicAndPersistsRequired() throws Exception {
        ...
        mockMvc.perform(
                        post(attachmentUrl(fixture.topic().id(), task.id()))
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(attachRequest(0, false)))
                .andExpect(status().isCreated())
        ...
        assertThat(topicTaskRepository.findAllByTopicIdOrderByPosition(fixture.topic().id()))
                .singleElement()
                .satisfies(link -> assertThat(link.required()).isFalse());
    }
    ...
    void listsOnlyTasksAttachedToOwnedTopicOrderedByPosition() throws Exception {
        ...
        TaskResult unrelated =
                createTask(other, "Unrelated", TaskDifficulty.MEDIUM, TaskStatus.DRAFT);
    ...
    void topicTaskListQueryCountDoesNotGrowWithTaskCount() throws Exception {
        ...
        for (int index = 0; index < 8; index++) {
            TaskResult task =
                    createTask(fixture, "Task " + index, TaskDifficulty.EASY, TaskStatus.DRAFT);
    ...
    void duplicateAttachmentAndPositionAreRejected() throws Exception {
        ...
        mockMvc.perform(
                        post(attachmentUrl(fixture.topic().id(), first.id()))
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(attachRequest(1, true)))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("TASK_ALREADY_ATTACHED"));

        mockMvc.perform(
                        post(attachmentUrl(fixture.topic().id(), second.id()))
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(attachRequest(0, true)))
                .andExpect(status().isConflict())
    ...
    void foreignTopicAndForeignTaskAreNotDisclosed() throws Exception {
        ...
        TaskResult foreignTask =
                createTask(foreign, "Foreign", TaskDifficulty.EASY, TaskStatus.DRAFT);

        mockMvc.perform(
                        post(attachmentUrl(foreign.topic().id(), ownerTask.id()))
                                .with(user(owner.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(attachRequest(0, true)))
        ...
        mockMvc.perform(
                        post(attachmentUrl(owner.topic().id(), foreignTask.id()))
                                .with(user(owner.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(attachRequest(0, true)))
    ...
    void teacherTaskApiEnforcesAuthenticationCsrfAndRole() throws Exception {
        ...
        mockMvc.perform(
                        post(tasksUrl())
                                ...
                                .content(
                                        createRequest(
                                                fixture.subject().id(), "No CSRF", "Text", "EASY")))
        ...
        UUID codeId = createCodeViaApi(fixture, 5000, 128);
        mockMvc.perform(
                        put(taskUrl(codeId) + "/programming-config")
                                .with(user(fixture.principal()))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(configUpdateRequest(5000, 128)))
                .andExpect(status().isForbidden())
    ...
    void openApiPublishesStableTaskOperationsAndSchemas() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/tasks'].post.operationId")
                                .value("createTask"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/tasks'].get.operationId")
                                .value("listTasks"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/tasks/{taskId}'].get.operationId")
                                .value("getTask"))
                .andExpect(
                        jsonPath("$.paths['/api/v1/teacher/tasks/{taskId}'].patch.operationId")
                                .value("updateTask"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/tasks/{taskId}/programming-config'].put.operationId")
                                .value("updateProgrammingTaskConfig"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/tasks/{taskId}/test-cases'].put.operationId")
                                .value("replaceTaskTestCases"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/topics/{topicId}/tasks'].get.operationId")
                                .value("listTopicTasks"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/topics/{topicId}/tasks/{taskId}'].post.operationId")
                                .value("attachTaskToTopic"))
                .andExpect(
                        jsonPath("$.components.schemas.CreateTaskRequest.properties.taskType")
                                .exists())
                .andExpect(
                        jsonPath("$.components.schemas.ProgrammingLanguage.enum[0]")
                                .value("PYTHON"))
                .andExpect(jsonPath("$.components.schemas.ComparisonMode.enum.length()").value(2))
                .andExpect(
                        jsonPath("$.components.schemas.TaskResponse.properties.id.format")
                                .value("uuid"))
                .andExpect(
                        jsonPath(
                                        "$.components.schemas.TaskResponse.properties.taskType.enum.length()")
                                .value(2))
                .andExpect(
                        jsonPath("$.components.schemas.TaskResponse.properties.createdAt.format")
                                .value("date-time"))
                .andExpect(
                        jsonPath(
                                        "$.paths['/api/v1/teacher/tasks'].post.responses['400'].content['*/*'].schema.$ref")
    ...
    private TaskFixture createFixture() {
        ...
        TeacherEntity teacher =
                teacherRepository.saveAndFlush(
                        new TeacherEntity(UUID.randomUUID(), userEntity, "Teacher"));
    ...
    private TaskResult createTask(
            TaskFixture fixture, String title, TaskDifficulty difficulty, TaskStatus desiredStatus)
            throws Exception {
        TaskResult task =
                taskService.createTask(
                        fixture.principal(),
                        new CreateTaskCommand(fixture.subject().id(), title, "Text", difficulty));
    ...
    private MvcResult createViaApi(TaskFixture fixture, String title, String difficulty)
            throws Exception {
    ...
    private UUID createCodeViaApi(TaskFixture fixture, int timeLimitMs, int memoryLimitMb)
            throws Exception {
        MvcResult result =
                mockMvc.perform(
                                post(tasksUrl())
    ...
    private void attach(TaskFixture fixture, UUID taskId, int position, boolean required)
            throws Exception {
        mockMvc.perform(
                        post(attachmentUrl(fixture.topic().id(), taskId))
                                .with(user(fixture.principal()))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(attachRequest(position, required)))
                .andExpect(status().isCreated());
    }

    private String createRequest(
            UUID subjectId, String title, String descriptionMarkdown, String difficulty)
            throws Exception {
        return objectMapper.writeValueAsString(
                new CreateBody(subjectId, title, descriptionMarkdown, difficulty));
    }
    ...
    private String updateRequest(
            ...
            throws Exception {
        return objectMapper.writeValueAsString(
                new UpdateBody(title, descriptionMarkdown, difficulty, status, version));
    }
