...
class StudentTopicTaskApiIntegrationTest extends PostgresIntegrationTest {
    ...
    void returnsOnlyActiveTextAndCodeTasksInTopicOrder() throws Exception {
        ...
        TaskEntity textTask = createTask(fixture, "Текстовая практика", TaskType.TEXT, TaskStatus.ACTIVE);
        ...
        mockMvc.perform(get(tasksUrl(fixture.program(), fixture.topic()))
    ...
    void codeTaskExposesPublicConfigurationAndTestsWithoutHiddenTestData() throws Exception {
        ...
        testCaseRepository.saveAllAndFlush(List.of(
            new TaskTestCase(
                publicTestId, codeTask.getId(), "2 3", "5", false, ComparisonMode.NORMALIZED, 0
            ),
            new TaskTestCase(
                hiddenTestId, codeTask.getId(), "hidden input", "DO_NOT_LEAK_EXPECTED_OUTPUT", true,
                ComparisonMode.EXACT, 1
            )
        ));
        attach(fixture.topic(), codeTask, 0, true);

        MvcResult result = mockMvc.perform(get(tasksUrl(fixture.program(), fixture.topic()))
                .with(user(fixture.principal())))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$[0].programmingConfig.language").value("PYTHON"))
            .andExpect(jsonPath("$[0].programmingConfig.starterCode").value("print('starter')"))
    ...
    void doesNotExposeForeignProgramOrTopicFromAnotherProgram() throws Exception {
        ...
        mockMvc.perform(get(tasksUrl(foreign.program(), foreign.topic()))
        ...
        mockMvc.perform(get(tasksUrl(current.program(), anotherCurrentProgram.topic()))
                .with(user(current.principal())))
            .andExpect(status().isNotFound())
    ...
    private Fixture createFixture(String label) {
        ...
        return createProgramFixture(new Fixture(
            teacher, student, subject, null, null, principal
        ), label);
    }
    ...
    private Fixture createProgramFixture(Fixture fixture, String label) {
        LearningProgramEntity learningProgram = learningProgramRepository.saveAndFlush(
            new LearningProgramEntity(
                ...
                "Program " + label, "Description", LearningProgramStatus.ACTIVE
            )
        );
        StudentProgramEntity program = studentProgramRepository.saveAndFlush(new StudentProgramEntity(
        ...
        ModuleEntity module = moduleRepository.saveAndFlush(new ModuleEntity(
            UUID.randomUUID(), learningProgram.getId(), "Module", "Description", 0
        ));
        TopicEntity topic = topicRepository.saveAndFlush(new TopicEntity(
            UUID.randomUUID(), module.id(), "Topic", "Description", 0, TopicStatus.ACTIVE
        ));
        return new Fixture(
            fixture.teacher(), fixture.student(), fixture.subject(), program, topic, fixture.principal()
        );
    }
    ...
    private TaskEntity createTask(Fixture fixture, String title, TaskType type, TaskStatus status) {
        return taskRepository.saveAndFlush(new TaskEntity(
            UUID.randomUUID(), fixture.teacher().id(), fixture.subject().id(), title,
            "Описание задания", type, TaskDifficulty.EASY, status
        ));
    }
    ...
    private TaskEntity createCodeTask(Fixture fixture, String title) {
        ...
        programmingConfigRepository.saveAndFlush(new ProgrammingTaskConfig(
            task.getId(), ProgrammingLanguage.PYTHON, "print('starter')", true, 5000, 128
        ));
        return task;
    }

    private void attach(TopicEntity topic, TaskEntity task, int position, boolean required) {
        topicTaskRepository.saveAndFlush(new TopicTaskEntity(topic.id(), task.getId(), position, required));
    }
