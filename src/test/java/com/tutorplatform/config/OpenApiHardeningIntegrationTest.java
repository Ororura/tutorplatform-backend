...
class OpenApiHardeningIntegrationTest extends PostgresIntegrationTest {
    ...
    private static final Set<String> SERVER_OWNED_REQUEST_FIELDS =
            Set.of(
                    "teacherId",
                    ...
                    "stdoutExcerpt",
                    "stderrExcerpt");
    private static final Set<String> INTERNAL_CONTRACT_FIELDS =
            Set.of("passwordHash", "sessionId", "tokenHash", "storageKey");
    ...
    void generatedContractHasUniqueOperationIdsAndExactCookieSecurity() throws Exception {
        ...
        JsonNode preview =
                document.required("paths")
                        .required("/api/v1/teacher/programs/{programId}/imports/preview")
                        .required("post");
        assertThat(preview.required("operationId").asText())
                .isEqualTo("previewTutorContentPackage");
        JsonNode upload =
                preview.required("requestBody").required("content").required("multipart/form-data");
        assertThat(upload.required("schema").toString()).contains("ContentPackagePreviewUpload");
        JsonNode uploadSchema =
                document.required("components")
                        .required("schemas")
                        .required("ContentPackagePreviewUpload");
        assertThat(uploadSchema.required("required").toString()).contains("file");
        assertThat(uploadSchema.required("properties").required("file").required("format").asText())
                .isEqualTo("binary");
        for (String code : Set.of("200", "400", "401", "403", "404", "409", "413")) {
            assertThat(preview.required("responses").has(code)).isTrue();
        }
        ...
        schemas.fields()
                .forEachRemaining(
                        schema -> {
                            ...
                            INTERNAL_CONTRACT_FIELDS.forEach(
                                    field ->
                                            assertThat(properties.has(field))
                                                    .as(
                                                            "%s must not export internal field %s",
                                                            schema.getKey(), field)
                                                    .isFalse());
                            if (!schema.getKey().endsWith("Request")) {
                                return;
                            }
                            SERVER_OWNED_REQUEST_FIELDS.forEach(
                                    field ->
                                            assertThat(properties.has(field))
                                                    .as(
                                                            "%s must not accept server-owned field %s",
                                                            schema.getKey(), field)
                                                    .isFalse());
                        });
    }
                ...
                .forEachRemaining(
                        schema -> {
                            ...
                            if ("string".equals(value.path("type").asText())) {
                                assertThat(
                                                value.path("enum").isArray()
                                                        && !value.path("enum").isEmpty())
                                        .as(
                                                "string component %s declares its enum values",
                                                schema.getKey())
                                        .isTrue();
                            }
