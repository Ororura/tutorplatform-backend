...
public class DemoDataSeedService {
    ...
    private SeedPeople seedPeople(Instant seedNow) {
        ...
        TeacherEntity teacher =
                teacherRepository.saveAndFlush(
                        new TeacherEntity(TEACHER, teacherUser, "Анна Смирнова"));
        StudentEntity alex =
                registeredStudent(ALEX_USER, ALEX, DemoDataAccess.ALEX_EMAIL, "Алексей", "Иванов");
        StudentEntity maria =
                registeredStudent(
                        MARIA_USER, MARIA, DemoDataAccess.MARIA_EMAIL, "Мария", "Петрова");
        StudentEntity ilya =
                studentRepository.saveAndFlush(
                        new StudentEntity(ILYA, "Илья", "Соколов", StudentStatus.ACTIVE));
        for (StudentEntity student : List.of(alex, maria, ilya)) {
            teacherStudentLinkRepository.saveAndFlush(
                    new TeacherStudentLinkEntity(teacher, student));
        }
        studentInviteRepository.saveAndFlush(
                new StudentInviteEntity(
                        ...
                        ilya,
                        teacher,
                        "ilya.demo@tutor.local",
                        tokenService.hash(DemoDataAccess.ILYA_INVITE_TOKEN),
                        seedNow.plus(30, ChronoUnit.DAYS)));
        return new SeedPeople(teacher, alex, maria, ilya);
    }

    private UserEntity user(UUID id, String email, String password, UserRole role) {
        UserEntity user =
                new UserEntity(id, email, passwordEncoder.encode(password), UserStatus.ACTIVE);
        user.addRole(role);
        return userRepository.saveAndFlush(user);
    }

    private StudentEntity registeredStudent(
            UUID userId, UUID studentId, String email, String firstName, String lastName) {
        UserEntity user = user(userId, email, DemoDataAccess.STUDENT_PASSWORD, UserRole.STUDENT);
        StudentEntity student =
                new StudentEntity(studentId, firstName, lastName, StudentStatus.ACTIVE);
    ...
    private SeedPrograms seedPrograms(SeedPeople people, Instant seedNow) {
        subjectRepository
                .findById(PYTHON_SUBJECT)
                .orElseThrow(
                        () ->
                                new IllegalStateException(
                                        "System PYTHON subject from V009 is missing"));

        LearningProgramEntity alexTemplate =
                learningProgramRepository.saveAndFlush(
                        new LearningProgramEntity(
                                ALEX_LEARNING_PROGRAM,
                                TEACHER,
                                PYTHON_SUBJECT,
                                "Python с нуля",
                                "Базовая программа с практикой и домашними заданиями",
                                LearningProgramStatus.ACTIVE));
        StudentProgramEntity alexProgram =
                studentProgramRepository.saveAndFlush(
                        new StudentProgramEntity(
                                ...
                                seedNow.minus(42, ChronoUnit.DAYS),
                                null));
        ...
        for (int moduleIndex = 0; moduleIndex < moduleTitles.length; moduleIndex++) {
            ModuleEntity module =
                    moduleRepository.saveAndFlush(
                            new ModuleEntity(
                                    ALEX_MODULES[moduleIndex],
                                    alexTemplate.getId(),
                                    moduleTitles[moduleIndex],
                                    null,
                                    moduleIndex));
            for (int topicIndex = 0; topicIndex < 3; topicIndex++) {
                ...
                alexTopics.add(
                        topicRepository.saveAndFlush(
                                new TopicEntity(
                                        ALEX_TOPICS[flatIndex],
                                        module.id(),
                                        topicTitles[moduleIndex][topicIndex],
                                        null,
                                        topicIndex,
                                        TopicStatus.ACTIVE)));
            }
        ...
        for (int index = 0; index < alexTopics.size(); index++) {
            ...
            Instant startedAt =
                    status == StudentTopicProgressStatus.LOCKED
                                    || status == StudentTopicProgressStatus.AVAILABLE
                            ? null
                            : seedNow.minus(35L - index * 3L, ChronoUnit.DAYS);
            Instant completedAt =
                    status == StudentTopicProgressStatus.COMPLETED
                            ? startedAt.plus(2, ChronoUnit.DAYS)
                            : null;
            topicProgressRepository.saveAndFlush(
                    new StudentTopicProgressEntity(
                            alexProgram.id(),
                            alexTopics.get(index).id(),
                            status,
                            startedAt,
                            completedAt));
        }

        LearningProgramEntity mariaTemplate =
                learningProgramRepository.saveAndFlush(
                        new LearningProgramEntity(
                                MARIA_LEARNING_PROGRAM,
                                TEACHER,
                                PYTHON_SUBJECT,
                                "Python — начало обучения",
                                "Короткая стартовая программа",
                                LearningProgramStatus.ACTIVE));
        StudentProgramEntity mariaProgram =
                studentProgramRepository.saveAndFlush(
                        new StudentProgramEntity(
                                MARIA_PROGRAM,
                                people.maria().getId(),
                                ...
                                StudentProgramStatus.ACTIVE,
                                REPORT_INTERVAL_MINUTES,
                                seedNow.minus(12, ChronoUnit.DAYS),
                                null));
        ModuleEntity mariaModule =
                moduleRepository.saveAndFlush(
                        new ModuleEntity(
    ...
    private void seedMaterials(SeedPrograms programs, TeacherEntity teacher) {
        record Material(
                int topic, LessonMaterialType type, String title, String content, String url) {}
        List<Material> materials =
                List.of(
                        new Material(
                                1,
                                LessonMaterialType.MARKDOWN,
                                "Краткий конспект: переменные",
                                "# Переменные\nИмя связывается со значением с помощью оператора `=`.",
                                null),
                        new Material(
                                4,
                                LessonMaterialType.TEXT,
                                "Основные правила работы с циклами",
                                "Используйте for для обхода последовательностей и range для диапазонов.",
                                null),
                        new Material(
                                0,
        ...
        for (int index = 0; index < materials.size(); index++) {
            ...
            materialRepository.saveAndFlush(
                    new LessonMaterialEntity(
                            MATERIALS[index],
                            programs.alexTopics().get(value.topic()).id(),
                            teacher.id(),
                            value.type(),
                            value.title(),
                            value.content(),
    ...
    private void seedSessionsAndAssessments(
        ...
        String[] summaries = {
            ...
            "Практиковали ввод, вывод и преобразование типов.",
            null,
            "Условия if/elif/else и логические выражения.",
            null,
        ...
        assessmentRepository.saveAndFlush(
                assessment(2, sessions.get(6), 5, 4, 5, null, "Уверенно использует for и range."));
        assessmentRepository.saveAndFlush(
                assessment(
                        3,
                        sessions.get(8),
        ...
        LessonSessionEntity mariaFirst =
                sessionRepository.saveAndFlush(
                        new LessonSessionEntity(
                                SESSIONS[9],
                                programs.mariaProgram().id(),
                                teacher.id(),
                                seedNow.minus(8, ChronoUnit.DAYS),
                                60,
                                AttendanceStatus.ATTENDED,
                                "Установили Python и написали первую программу.",
                                null));
        sessionTopicRepository.saveAndFlush(
                new LessonSessionTopicEntity(
    ...
    private List<TaskEntity> seedTasks(SeedPrograms programs, TeacherEntity teacher) {
        ...
        for (int index = 0; index < 8; index++) {
            ...
            topicTaskRepository.saveAndFlush(
                    new TopicTaskEntity(
                            programs.alexTopics().get(Math.min(index, 8)).id(),
                            task.getId(),
                            0,
                            true));
        }
    ...
    private void seedHomeworkAndSubmissions(
        ...
        HomeworkEntity h1 =
                homework(
                        0,
                        programs.alexProgram(),
                        "Основы Python",
                        -30,
                        -23,
                        HomeworkStatus.COMPLETED,
                        seedNow.minus(22, ChronoUnit.DAYS),
                        seedNow,
                        List.of(
                                hwItem(item++, 0, true, tasks),
                                hwItem(item++, 4, true, tasks),
        ...
        HomeworkEntity h3 =
                homework(
                        ...
                        seedNow,
                        List.of(
                                hwItem(item++, 3, true, tasks),
                                hwItem(item++, 5, true, tasks),
        ...
        HomeworkEntity mariaHomework =
                homework(
                        5,
                        programs.mariaProgram(),
                        "Первое задание",
                        -4,
                        3,
                        HomeworkStatus.ASSIGNED,
        ...
        textSubmission(
                ...
                "= присваивает значение, а == сравнивает два значения.",
                -28,
                seedNow);
        codeSubmission(
                submission++,
        ...
        textSubmission(
                ...
                "Список изменяемый, кортеж неизменяемый; кортеж удобен для фиксированных данных.",
                -26,
                seedNow);
        textSubmission(
                submission++,
        ...
        textSubmission(
                ...
                1,
                SubmissionStatus.NEEDS_REVIEW,
                "while нужен, когда заранее неизвестно количество повторов.",
                -4,
                seedNow);
        codeSubmission(
                submission++,
        ...
        textSubmission(
                ...
                2,
                SubmissionStatus.PASSED,
                "= связывает имя со значением, == проверяет равенство.",
                -14,
                seedNow);
        codeSubmission(
                submission++,
        ...
        textSubmission(
                ...
                programs.mariaProgram(),
                tasks.get(0),
                mariaHomework.getItems().get(0),
                1,
                SubmissionStatus.NEEDS_REVIEW,
                "= записывает значение, == проверяет, одинаковые ли значения.",
                -1,
                seedNow);
    }
    ...
    private HomeworkEntity homework(
        ...
        return homeworkRepository.saveAndFlush(
                new HomeworkEntity(
                        ...
                        seedNow.plus(assignedDays, ChronoUnit.DAYS),
                        seedNow.plus(dueDays, ChronoUnit.DAYS),
                        status,
                        completedAt,
                        items));
    }

    private HomeworkItemEntity hwItem(
            int itemIndex, int taskIndex, boolean required, List<TaskEntity> tasks) {
        int homeworkIndex;
        int position;
        if (itemIndex < 3) {
            homeworkIndex = 0;
            ...
            homeworkIndex = 1;
            position = itemIndex - 3;
        } else if (itemIndex < 8) {
            homeworkIndex = 2;
            position = itemIndex - 5;
        } else if (itemIndex < 10) {
            homeworkIndex = 3;
            position = itemIndex - 8;
        } else if (itemIndex < 12) {
            homeworkIndex = 4;
            position = itemIndex - 10;
        } else {
            homeworkIndex = 5;
            position = itemIndex - 12;
        }
        return new HomeworkItemEntity(
                HOMEWORK_ITEMS[itemIndex],
                HOMEWORK[homeworkIndex],
                tasks.get(taskIndex).getId(),
                position,
                required);
    }
    ...
    private void codeSubmission(
        ...
        codeSubmissionRepository.saveAndFlush(
                new CodeSubmissionEntity(
                        ...
                        executionStatus == CodeExecutionStatus.RUNTIME_ERROR
    ...
    private void seedPeriodsReportsAndShares(
        ...
        LearningPeriod alexActive =
                LearningPeriod.active(
                                ...
                                300,
                                REPORT_INTERVAL_MINUTES,
                                periodCompleted.plusSeconds(1))
                        .withStartedAt(
                                seedNow.minus(9, ChronoUnit.DAYS),
                                seedNow.minus(9, ChronoUnit.DAYS));
        learningPeriodRepository.saveAndFlush(alexActive);
        LearningPeriod mariaPeriod =
                LearningPeriod.active(
                                MARIA_ACTIVE_PERIOD,
                                programs.mariaProgram().id(),
                                1,
        ...
        ProgressReport report =
                progressReportRepository.saveAndFlush(
                        new ProgressReport(
                                PUBLISHED_REPORT,
                                programs.alexProgram().id(),
                                ALEX_COMPLETED_PERIOD,
                                ...
                                snapshotFactory.create(
                                        currentProgressService.getProgressSnapshot(
                                                programs.alexProgram().id(),
                                                new ProgressInterval(periodStart, periodCompleted)),
                                        300),
        ...
        progressShareRepository.saveAndFlush(
                new ProgressShare(
                        ...
                        programs.alexProgram().id(),
                        teacher.id(),
                        tokenService.hash(DemoDataAccess.PROGRESS_TOKEN),
                        null,
                        null,
                        seedNow));
        reportShareRepository.saveAndFlush(
                new ReportShare(
                        REPORT_SHARE,
                        report.id(),
                        teacher.id(),
                        tokenService.hash(DemoDataAccess.REPORT_TOKEN),
                        null,
                        null,
                        seedNow));
        reportShareRepository.saveAndFlush(
                new ReportShare(
                        EXPIRED_REPORT_SHARE,
                        report.id(),
                        teacher.id(),
                        tokenService.hash(DemoDataAccess.EXPIRED_REPORT_TOKEN),
                        seedNow.minus(1, ChronoUnit.DAYS),
                        null,
                        seedNow.minus(10, ChronoUnit.DAYS)));
    }
    ...
    private SeedCounts validateSeededDataset() {
        SeedCounts counts =
                new SeedCounts(
                        count("teachers"),
                        count("students"),
                        count("student_programs"),
                        count("modules"),
                        count("topics"),
                        count("lesson_materials"),
                        count("lesson_sessions"),
                        count("teacher_assessments"),
                        count("tasks"),
                        count("homeworks"),
                        count("submissions"),
                        count("learning_periods"),
                        countWhere("progress_reports", "status = 'PUBLISHED'"),
                        count("progress_shares"),
                        count("report_shares"));
    ...
    private record SeedPeople(
            TeacherEntity teacher, StudentEntity alex, StudentEntity maria, StudentEntity ilya) {}

    private record SeedPrograms(
            StudentProgramEntity alexProgram,
            StudentProgramEntity mariaProgram,
            List<TopicEntity> alexTopics,
            List<TopicEntity> mariaTopics) {}
