...
public class JpaHomeworkRepository implements HomeworkRepository {
    ...
    public Optional<HomeworkEntity> findByHomeworkItemIdWithItems(UUID homeworkItemId) {
        return databaseRepository
                .findWithItemsByHomeworkItemId(homeworkItemId)
                .map(HomeworkDatabaseModel::toEntity);
    }

    @Override
    public List<HomeworkEntity> findAllByStudentProgramId(
            UUID studentProgramId, int page, int size) {
        if (page < 0) {
        ...
        return databaseRepository
                .findAllByStudentProgramIdOrderByAssignedAtDesc(
    ...
    public Optional<HomeworkItemEntity> findItemById(UUID homeworkId, UUID homeworkItemId) {
        return itemDatabaseRepository
                .findByHomeworkIdAndId(homeworkId, homeworkItemId)
