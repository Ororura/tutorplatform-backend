...
public record StudentHomeworkDetailsResponse(
        ...
        @Schema(requiredMode = Schema.RequiredMode.REQUIRED) List<StudentHomeworkItemResponse> items
) {
    public static StudentHomeworkDetailsResponse from(StudentHomeworkDetailsResult homework) {
        return new StudentHomeworkDetailsResponse(
                ...
                homework.status(), homework.assignedAt(), homework.dueAt(), homework.overdue(),
                homework.completedAt(), homework.items().stream().map(StudentHomeworkItemResponse::from).toList()
        );
    }
}
