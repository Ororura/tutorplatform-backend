...
public class FileMaterialPolicy {
    ...
    private static final Map<String, Set<String>> FILE_MIME_TYPES =
            Map.ofEntries(
                    ...
                    Map.entry(
                            ".tsx",
                            withTextMimes("application/typescript", "text/typescript", "text/tsx")),
    ...
    public byte[] readAndValidate(
        ...
        // Extension, declared MIME and content must all agree. Browser MIME is never trusted by
        // itself.
        if (mimeType == null
                || !allowedMimeTypes.contains(mimeType)
                || !isAllowedCombination(extension, mimeType, type)
                || !matches(content, extension)) {
            throw new InvalidLessonMaterialException("file", "unsupported or mismatched MIME type");
        }
        return content;
    }

    private boolean isAllowedCombination(
            String extension, String mimeType, LessonMaterialType type) {
    ...
    private boolean matches(byte[] bytes, String extension) {
        return switch (extension) {
            ...
            case ".jpg", ".jpeg" ->
                    startsWith(bytes, new byte[] {(byte) 255, (byte) 216, (byte) 255});
            case ".zip" -> isZip(bytes);
            case ".txt", ".md", ".csv", ".json", ".py", ".sh", ".js", ".ts", ".tsx", ".java" ->
                    isText(bytes);
            default -> false;
        };
    }
