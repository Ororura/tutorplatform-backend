...
public class CodeSubmissionTransactions {
    ...
    public SubmissionResult createPending(
        ...
        SubmissionAttemptContext context = new SubmissionAttemptContext(
            studentId, studentProgramId, taskId, homeworkItemId
        );
        SubmissionEntity submission = submissionRepository.saveAndFlush(new SubmissionEntity(
            UUID.randomUUID(), studentId, studentProgramId, taskId, homeworkItemId,
            submissionRepository.nextAttemptNo(context), SubmissionStatus.SUBMITTED,
            null, Instant.now()
        ));
        CodeSubmissionEntity code = codeSubmissionRepository.saveAndFlush(
    ...
    public SubmissionResult finish(
        ...
        CodeSubmissionEntity code = codeSubmissionRepository.findBySubmissionId(submissionId)
            .orElseThrow(SubmissionNotFoundException::new);
        SubmissionStatus submissionStatus = switch (executionStatus) {
            ...
            case PENDING, RUNNING -> throw new IllegalArgumentException("A final execution status is required");
        };
