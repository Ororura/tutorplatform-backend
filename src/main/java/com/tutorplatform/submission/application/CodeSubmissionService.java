...
public class CodeSubmissionService {
    ...
    public SubmissionResult submit(
        ...
        try {
            executionResult = executionPort.execute(toExecutionRequest(executionId, sourceCode, codeTask));
            ...
            log.warn(
                ...
                pending.id(), exception.getClass().getSimpleName()
            );
            executionResult = ExecutionResult.systemError(executionId, configuredTestCount);
        }
    ...
    private SubmissionHomeworkContextQuery.HomeworkSubmissionContext requireOwnedHomeworkContext(
        ...
        var homework = homeworkContextQuery.findSubmissionContext(homeworkItemId)
    ...
    private void requireExecutable(TaskQuery.CodeTaskConfiguration task) {
        if (task.type() != TaskType.CODE || task.status() != TaskStatus.ACTIVE
            ...
            throw new InvalidSubmissionException("taskId", "must reference an active executable CODE task");
        }
    ...
    private ExecutionRequest toExecutionRequest(
        ...
        return new ExecutionRequest(
            executionId, ExecutionLanguage.valueOf(config.language().name()), sourceCode,
            config.timeLimitMs(), config.memoryLimitMb(),
            task.testCases().stream().map(test -> new ExecutionTestCase(
                test.id(), test.inputText(), test.expectedOutput(),
                ExecutionComparisonMode.valueOf(test.comparisonMode().name())
            )).toList()
        );
    }
