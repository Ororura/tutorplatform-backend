...
public class JpaSubmissionQuery implements SubmissionQuery {
    ...
    public Map<UUID, HomeworkItemSubmissionState> findHomeworkItemStates(
        ...
        for (Object[] row : databaseRepository.findHomeworkItemSubmissionStates(
            studentId, studentProgramId, homeworkItemIds
        )) {
            UUID homeworkItemId = (UUID) row[0];
            SubmissionStatus status = (SubmissionStatus) row[1];
            MutableHomeworkItemState state = states.computeIfAbsent(
        ...
        return states.entrySet().stream().collect(Collectors.toUnmodifiableMap(
            Map.Entry::getKey,
            entry -> new HomeworkItemSubmissionState(
                entry.getValue().passed, entry.getValue().latestSubmissionStatus
            )
        ));
    }
