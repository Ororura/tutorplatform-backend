...
public class JdbcTeacherLearningProgramQuery implements TeacherLearningProgramQuery {
    ...
    public List<LearningProgramSummary> findPrograms(UUID teacherId, LearningProgramStatus status) {
        ...
        JdbcClient.StatementSpec statement =
            ...
            """
                                        + statusClause
                                        + " ORDER BY program.created_at DESC, program.id ASC")
                ...
                .map(
                        program ->
                                new LearningProgramDetails(
                                        ...
                                        program.createdAt(),
                                        program.updatedAt(),
                                        hasAssignments(program.id()),
                                        findModules(program.id())));
    }
    ...
    private List<ModuleDetails> findModules(UUID learningProgramId) {
        ...
        Map<UUID, List<TopicDetails>> topicsByModuleId =
                jdbcClient
                        .sql(
                                """
                        ...
                        .query(
                                (resultSet, rowNumber) ->
                                        new TopicRow(
                                                ...
                                                resultSet.getString("description"),
                                                resultSet.getInt("position"),
                                                TopicStatus.valueOf(resultSet.getString("status")),
                                                resultSet.getObject("version", Long.class)))
                        .list()
                        .stream()
                        .collect(
