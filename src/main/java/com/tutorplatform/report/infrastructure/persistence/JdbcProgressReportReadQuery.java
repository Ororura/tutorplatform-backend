...
public class JdbcProgressReportReadQuery implements ProgressReportReadQuery {
    ...
    public ProgressReportSummaryPage findOwnedPage(
        ...
        StringBuilder fromAndFilters = new StringBuilder("""
        ...
        List<ProgressReportSummary> items = jdbc.query(
            """
                ...
                """ + fromAndFilters
                + " order by " + sortColumn + direction + ", report.id desc limit :limit offset :offset",
            parameters,
            (row, rowNumber) -> new ProgressReportSummary(
                ...
                row.getTimestamp("published_at") == null
                    ? null : row.getTimestamp("published_at").toInstant(),
                row.getTimestamp("created_at").toInstant(),
                row.getTimestamp("updated_at").toInstant()
            )
        );
        long totalElements = jdbc.queryForObject(
            "select count(*) " + fromAndFilters, parameters, Long.class
        );
