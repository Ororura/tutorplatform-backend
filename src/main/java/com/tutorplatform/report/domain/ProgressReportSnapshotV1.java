...
public record ProgressReportSnapshotV1(
    ...
    public record Metrics(
        ...
        public Metrics {
            if (learningMinutes < 0 || sessionsCount < 0 || homeworkAssigned < 0
                || homeworkCompleted < 0 || practiceAssigned < 0 || practiceCompleted < 0) {
                throw new IllegalArgumentException("progress metrics must not be negative");
            }
            if (!Double.isFinite(attendanceRate)
                || attendanceRate < 0.0 || attendanceRate > 1.0) {
