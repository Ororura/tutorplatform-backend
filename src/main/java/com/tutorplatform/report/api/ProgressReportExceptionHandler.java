...
public class ProgressReportExceptionHandler {
    ...
    ResponseEntity<ApiError> handleInvalidList(
            InvalidProgressReportListParameterException exception) {
    ...
    ResponseEntity<ApiError> handleLearningPeriodNotFound(
            LearningPeriodNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(
                        ApiError.of(
                                "LEARNING_PERIOD_NOT_FOUND",
                                "Learning period not found",
                                MDC.get("traceId")));
    }
    ...
    ResponseEntity<ApiError> handleInvalidPeriod(InvalidProgressReportPeriodException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
                                "PROGRESS_REPORT_PERIOD_INVALID",
    ...
    ResponseEntity<ApiError> handleNotPublishable(ProgressReportNotPublishableException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
                                "PROGRESS_REPORT_NOT_PUBLISHABLE",
                                exception.getMessage(),
                                MDC.get("traceId")));
    }

    @ExceptionHandler(ProgressReportPdfNotAvailableException.class)
    ResponseEntity<ApiError> handlePdfNotAvailable(
            ProgressReportPdfNotAvailableException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
                                "PROGRESS_REPORT_PDF_NOT_AVAILABLE",
                                exception.getMessage(),
                                MDC.get("traceId")));
    }
    ...
    ResponseEntity<ApiError> handlePdfRenderFailure(ProgressReportPdfRenderException exception) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(
                        ApiError.of(
                                "PROGRESS_REPORT_PDF_FAILED",
                                "Progress report PDF generation failed",
                                MDC.get("traceId")));
    }

    @ExceptionHandler(UnsupportedProgressReportSnapshotException.class)
    ResponseEntity<ApiError> handleUnsupportedSnapshot(
            UnsupportedProgressReportSnapshotException exception) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(
                        ApiError.of(
    ...
    ResponseEntity<ApiError> handleVersionConflict(
            ProgressReportVersionConflictException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
    ...
    ResponseEntity<ApiError> handleInvalidShareExpiration(
            InvalidReportShareExpirationException exception) {
        return ResponseEntity.badRequest()
                .body(
                        ApiError.of(
                                "REPORT_SHARE_EXPIRATION_INVALID",
    ...
    ResponseEntity<ApiError> handleShareNotAllowed(ReportShareNotAllowedException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
                                "REPORT_SHARE_NOT_ALLOWED",
                                "Only published progress reports can be shared",
                                MDC.get("traceId")));
    }
