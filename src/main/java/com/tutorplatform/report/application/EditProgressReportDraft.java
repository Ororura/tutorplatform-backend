...
public class EditProgressReportDraft {
    ...
    private ProgressReport editCurrent(
        ...
        try {
            ProgressReport edited = current.editDraft(
                command.teacherSummaryPresent() ? command.teacherSummary() : current.teacherSummary(),
                command.nextPeriodPlanPresent() ? command.nextPeriodPlan() : current.nextPeriodPlan(),
                command.version(),
                Instant.now()
            );
