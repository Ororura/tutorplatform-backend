...
public class LearningPeriodService {
    ...
    private LearningPeriod recalculate(LessonSessionChangedEvent event) {
        ...
        int totalLearningMinutes =
                Math.toIntExact(
                        progressReadRepository
                                .getSessionMetrics(studentProgram.id())
        ...
        LearningPeriod completed =
                learningPeriodRepository.saveAndFlush(
                        recalculated.complete(
                                totalLearningMinutes, timeline.thresholdCrossedAt(), now));
        learningPeriodRepository.saveAndFlush(
    ...
    public List<LearningPeriod> listLearningPeriods(UUID studentProgramId) {
        return learningPeriodRepository.findAllByStudentProgramIdOrderBySequenceNo(
                studentProgramId);
    }

    private ProgramQuery.StudentProgramContext lockStudentProgram(UUID studentProgramId) {
        return programQuery
                .findStudentProgramForUpdate(studentProgramId)
                .orElseThrow(
                        () -> new LearningPeriodStudentProgramNotFoundException(studentProgramId));
    }

    private LearningPeriod ensureActivePeriod(ProgramQuery.StudentProgramContext studentProgram) {
        return learningPeriodRepository
                .findActiveByStudentProgramIdForUpdate(studentProgram.id())
                .orElseGet(
                        () -> {
                            LearningPeriod latest =
                            ...
                            Instant now = Instant.now();
                            return learningPeriodRepository.saveAndFlush(
                                    LearningPeriod.active(
                                            UUID.randomUUID(),
                                            studentProgram.id(),
                                            sequenceNo,
    ...
    private int completedEnd(LearningPeriod latest) {
        if (latest.status() != LearningPeriodStatus.COMPLETED) {
            throw new IllegalStateException(
                    "Latest LearningPeriod is neither ACTIVE nor COMPLETED");
        }
        return latest.endCumulativeMinutes();
    }

    private Timeline buildTimeline(
            LearningPeriod active, List<AttendedLessonSession> sessions, int expectedTotal) {
        int cumulative = 0;
