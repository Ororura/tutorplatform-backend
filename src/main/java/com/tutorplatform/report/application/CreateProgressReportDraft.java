...
public class CreateProgressReportDraft {
    ...
    private ProgressReport create(
        ...
        LearningPeriod period = learningPeriodRepository.findById(learningPeriodId)
        ...
        int learningMinutes = Math.subtractExact(
            period.endCumulativeMinutes(), period.startCumulativeMinutes()
        );
        CurrentProgress progress = progressService.getProgressSnapshot(
            studentProgramId, new ProgressInterval(period.startedAt(), period.completedAt())
        );
        Instant now = Instant.now();
        return writer.save(ProgressReport.draft(
            UUID.randomUUID(), studentProgramId, learningPeriodId, authorized.teacherId(),
            period.startedAt(), period.completedAt(), learningMinutes,
            snapshotFactory.create(progress, learningMinutes), now
        ));
    }
    ...
    private void validatePeriod(LearningPeriod period, UUID studentProgramId) {
        ...
        if (period.startedAt() == null || period.completedAt() == null
