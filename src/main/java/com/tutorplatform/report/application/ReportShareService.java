...
public class ReportShareService {
    ...
    public ReportShareCreatedResponse create(
            AuthenticatedUser principal, UUID reportId, CreateReportShareRequest request) {
        ...
        return new ReportShareCreatedResponse(
                ...
                share.reportId(),
                share.expiresAt(),
                publicFrontendBaseUrl + "/reports/" + token.rawValue(),
                share.createdAt());
    }
    ...
    public ReportShareListResponse list(AuthenticatedUser principal, UUID reportId) {
        ProgressReport report = reportQueryService.get(principal, reportId);
        Instant now = Instant.now();
        return new ReportShareListResponse(
                reportShareRepository.listByReportId(report.id()).stream()
                        .map(
                                share ->
                                        new ReportShareSummaryResponse(
                                                share.id(),
                                                share.reportId(),
                                                share.statusAt(now),
                                                share.expiresAt(),
    ...
    public void revoke(AuthenticatedUser principal, UUID reportId, UUID shareId) {
        ...
        ReportShare share =
                reportShareRepository
                        .findByIdAndReportId(shareId, report.id())
