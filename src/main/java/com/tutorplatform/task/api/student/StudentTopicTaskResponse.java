...
public record StudentTopicTaskResponse(
    ...
    public static StudentTopicTaskResponse from(StudentTopicTaskResult task) {
        return new StudentTopicTaskResponse(
            ...
            task.position(),
            task.required(),
            task.programmingConfig() == null ? null : ProgrammingConfigResponse.from(task.programmingConfig()),
            task.testCases() == null ? null : task.testCases().stream().map(PublicTestCaseResponse::from).toList()
        );
    }
