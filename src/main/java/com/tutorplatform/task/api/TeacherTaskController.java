...
public class TeacherTaskController implements TeacherTaskApi {
        ...
        var created =
                taskService.createTask(
                        ...
                        new CreateTaskCommand(
                                ...
                                request.difficulty(),
                                request.taskType(),
                                request.programmingConfig() == null
                                        ? null
                                        : new ProgrammingTaskConfigInput(
                                                request.programmingConfig().language(),
                                                request.programmingConfig().starterCode(),
                                                request.programmingConfig().executionEnabled(),
                                                request.programmingConfig().timeLimitMs(),
    ...
    public ProgrammingTaskConfigResponse updateProgrammingTaskConfig(
        ...
        return ProgrammingTaskConfigResponse.from(
                taskService.updateProgrammingTaskConfig(
                        principal,
                        taskId,
    ...
    public TaskTestCasesResponse replaceTaskTestCases(
        ...
        return TaskTestCasesResponse.from(
                taskService.replaceTaskTestCases(
                        principal,
                        taskId,
    ...
    public TaskPageResponse listTasks(
        ...
        return TaskPageResponse.from(
                taskService.listTasks(principal, subjectId, status, difficulty, page, size, sort));
    }

    @Override
    @GetMapping("/tasks/{taskId}")
    public TaskResponse getTask(
            @AuthenticationPrincipal AuthenticatedUser principal, @PathVariable UUID taskId) {
    ...
    public List<TopicTaskDetailsResponse> listTopicTasks(
            @AuthenticationPrincipal AuthenticatedUser principal, @PathVariable UUID topicId) {
    ...
    public ResponseEntity<TopicTaskResponse> attachTaskToTopic(
        ...
        var attached =
                taskService.attachTaskToTopic(
                        principal,
                        topicId,
                        taskId,
                        new AttachTaskToTopicCommand(request.position(), request.required()));
        return ResponseEntity.created(
                        URI.create("/api/v1/teacher/topics/" + topicId + "/tasks/" + taskId))
