...
public class TaskExceptionHandler {
    ...
    ResponseEntity<ApiError> handleInvalidTask(RuntimeException exception) {
        String field =
                exception instanceof InvalidTaskException invalid
                        ? invalid.getField()
    ...
    ResponseEntity<ApiError> handleInvalidProgrammingConfig(
            InvalidProgrammingTaskConfigException exception) {
        return ResponseEntity.badRequest()
                .body(
                        new ApiError(
                                "PROGRAMMING_CONFIG_INVALID",
                                "Programming task configuration is invalid",
                                Instant.now(),
                                MDC.get("traceId"),
                                List.of(
                                        new ApiErrorDetail(
    ...
    ResponseEntity<ApiError> handleInvalidTestCase(InvalidTaskTestCaseException exception) {
        return ResponseEntity.badRequest()
                .body(
                        new ApiError(
                                "TASK_TEST_CASE_INVALID",
                                "Task test case is invalid",
                                Instant.now(),
                                MDC.get("traceId"),
                                List.of(
                                        new ApiErrorDetail(
    ...
    ResponseEntity<ApiError> handleTaskTypeMismatch(TaskTypeMismatchException exception) {
        return ResponseEntity.badRequest()
                .body(
                        ApiError.of(
                                "TASK_TYPE_MISMATCH", exception.getMessage(), MDC.get("traceId")));
    }
    ...
    ResponseEntity<ApiError> handleTaskNotFound(TaskNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiError.of("TASK_NOT_FOUND", "Task not found", MDC.get("traceId")));
    }
    ...
    ResponseEntity<ApiError> handleTaskTopicNotFound(TaskTopicNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiError.of("TOPIC_NOT_FOUND", "Topic not found", MDC.get("traceId")));
    }
    ...
    ResponseEntity<ApiError> handleTaskTopicPositionConflict(
            TaskTopicPositionConflictException exception) {
    ...
    ResponseEntity<ApiError> handleTaskAlreadyAttached(TaskAlreadyAttachedException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
                                "TASK_ALREADY_ATTACHED",
                                "Task is already attached to this topic",
                                MDC.get("traceId")));
    }
