...
public class TaskService {
    ...
    private static final Set<String> ALLOWED_SORT_FIELDS =
            Set.of("createdAt", "updatedAt", "title", "difficulty");
    ...
    public TaskResult createTask(AuthenticatedUser principal, CreateTaskCommand command) {
        ...
        if (taskType == TaskType.TEXT
                && (command.programmingConfig() != null || command.testCases() != null)) {
        ...
        if (taskType == TaskType.CODE) {
            ...
            List<TaskTestCase> testCases =
                    createTestCases(task.getId(), command.testCases(), false);
    ...
    public TaskResult updateTask(
            AuthenticatedUser principal, UUID taskId, UpdateTaskCommand command) {
        UUID teacherId = currentTeacherId(principal);
    ...
    public ProgrammingTaskConfig updateProgrammingTaskConfig(
            AuthenticatedUser principal, UUID taskId, UpdateProgrammingTaskConfigCommand command) {
        requireOwnedCodeTask(taskId, currentTeacherId(principal));
        ProgrammingTaskConfig current =
    ...
    public List<TaskTestCase> replaceTaskTestCases(
            AuthenticatedUser principal, UUID taskId, List<TaskTestCaseInput> inputs) {
        requireOwnedCodeTask(taskId, currentTeacherId(principal));
        ...
        for (int index = 0; index < replacements.size(); index++) {
            ...
            if (input.id() != null && !existing.containsKey(input.id())) {
                throw new InvalidTaskTestCaseException(
                        "items[" + index + "].id", "does not belong to this task");
            }
            ...
            if (previous != null) {
                replacements.set(
                        ...
                        new TaskTestCase(
                                ...
                                replacement.expectedOutput(),
                                replacement.hidden(),
                                replacement.comparisonMode(),
    ...
    public TopicTaskResult attachTaskToTopic(
        ...
        ProgramQuery.TopicContext topic =
                programQuery.findTopic(topicId).orElseThrow(TaskTopicNotFoundException::new);
    ...
    private TaskEntity requireOwnedTask(UUID taskId, UUID teacherId) {
        TaskEntity task =
                taskRepository
                        .findOwnedById(taskId, teacherId)
    ...
    private ProgrammingTaskConfig createConfig(UUID taskId, ProgrammingTaskConfigInput input) {
        if (input == null) {
            throw new InvalidProgrammingTaskConfigException(
                    "programmingConfig", "is required for CODE tasks");
        }
        if (input.language() == null) {
            throw new InvalidProgrammingTaskConfigException(
                    "programmingConfig.language", "is required");
        }
        if (input.executionEnabled() == null
                || input.timeLimitMs() == null
                || input.memoryLimitMb() == null) {
            throw new InvalidProgrammingTaskConfigException(
                    "programmingConfig", "all limits and executionEnabled are required");
        }
        try {
    ...
    private InvalidProgrammingTaskConfigException invalidConfig(
            IllegalArgumentException exception) {
        String field =
                exception.getMessage().startsWith("timeLimitMs") ? "timeLimitMs" : "memoryLimitMb";
        return new InvalidProgrammingTaskConfigException(field, exception.getMessage());
    }

    private List<TaskTestCase> createTestCases(
            UUID taskId, List<TaskTestCaseInput> inputs, boolean preserveIds) {
        ...
        for (int index = 0; index < inputs.size(); index++) {
            ...
            if (input.hidden() == null
                    ...
                    || input.position() == null) {
                throw new InvalidTaskTestCaseException(
                        prefix, "hidden, comparisonMode, and position are required");
            }
            if (!positions.add(input.position())) {
                throw new InvalidTaskTestCaseException(
                        prefix + ".position", "must be unique within the task");
            }
            try {
                result.add(
                        new TaskTestCase(
                ...
                throw new InvalidTaskTestCaseException(
                        prefix + ".position", exception.getMessage());
            }
        }
        return result;
    }
    ...
    private String validateAndNormalizeTask(
            String title, String descriptionMarkdown, TaskDifficulty difficulty) {
    ...
    private TaskResult toResult(TaskEntity task) {
        ProgrammingTaskConfig config =
                task.getTaskType() == TaskType.CODE
                        ? programmingConfigRepository.findByTaskId(task.getId()).orElse(null)
                        : null;
        List<TaskTestCase> testCases =
                task.getTaskType() == TaskType.CODE
                        ? testCaseRepository.findAllByTaskId(task.getId())
                        : null;
