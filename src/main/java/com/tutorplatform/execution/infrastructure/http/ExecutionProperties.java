    ...
    public record Worker(
        ...
        public Worker {
            if (baseUrl != null && (!baseUrl.isAbsolute() || baseUrl.getHost() == null)) {
                throw new IllegalArgumentException("execution.worker.base-url must be an absolute HTTP URL");
            }
            if (baseUrl != null && !"http".equals(baseUrl.getScheme()) && !"https".equals(baseUrl.getScheme())) {
                throw new IllegalArgumentException("execution.worker.base-url must use HTTP or HTTPS");
            }
        ...
        private static void requirePositiveBounded(Duration value, String name) {
            if (value != null && (value.isZero() || value.isNegative() || value.compareTo(MAX_HTTP_TIMEOUT) > 0)) {
