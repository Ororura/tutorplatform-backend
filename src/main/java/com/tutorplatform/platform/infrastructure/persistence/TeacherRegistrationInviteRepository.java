...
public class TeacherRegistrationInviteRepository {
    ...
    public TeacherRegistrationInviteRepository(JdbcClient jdbcClient) {
        this.jdbcClient = jdbcClient;
    }

    public void create(UUID id, UUID adminId, String email, String tokenHash, Instant expiresAt) {
        int inserted =
                jdbcClient
                        .sql(
                                """
        ...
        if (inserted != 1) {
            throw new IllegalStateException("Failed to create teacher registration invitation");
        }
    }

    public Optional<TeacherRegistrationInvite> findByTokenHash(String tokenHash) {
        return jdbcClient
                .sql(
                        SELECT_INVITE
                                + """
    ...
    public Optional<TeacherRegistrationInvite> findByTokenHashForUpdate(String tokenHash) {
        return jdbcClient
                .sql(
                        SELECT_INVITE
                                + """
    ...
    public List<TeacherRegistrationInvite> findAllByAdminId(UUID adminId) {
        return jdbcClient
                .sql(
                        SELECT_INVITE
                                + """
    ...
    public Optional<TeacherRegistrationInvite> findByIdAndAdminId(UUID invitationId, UUID adminId) {
        return jdbcClient
                .sql(
                        SELECT_INVITE
                                + """
    ...
    public int revokeActive(UUID invitationId, UUID adminId) {
        return jdbcClient
                .sql(
                        """
    ...
    public int markAccepted(UUID invitationId) {
        return jdbcClient
                .sql(
                        """
                UPDATE teacher_registration_invites
    ...
    private static TeacherRegistrationInvite mapRow(ResultSet rs, int rowNum) throws SQLException {
