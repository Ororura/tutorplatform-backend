...
public interface StudentInviteRepository extends JpaRepository<StudentInviteEntity, UUID> {
    ...
    Optional<StudentInviteEntity> findLockedByTokenHash(String tokenHash);

    List<StudentInviteEntity> findAllByStudent_IdOrderByCreatedAtDesc(UUID studentId);

    Optional<StudentInviteEntity> findByIdAndStudent_Id(UUID id, UUID studentId);
    ...
    int revokeActiveInvites(
            @Param("studentId") UUID studentId, @Param("revokedAt") Instant revokedAt);
}
