...
public class StudentExceptionHandler {
    ...
    ResponseEntity<ApiError> handleInvalidStudentListParameter(
            InvalidStudentListParameterException exception) {
    ...
    ResponseEntity<ApiError> handleStudentNotFound(StudentNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiError.of("STUDENT_NOT_FOUND", "Student not found", MDC.get("traceId")));
    }

    @ExceptionHandler(StudentAlreadyRegisteredException.class)
    ResponseEntity<ApiError> handleStudentAlreadyRegistered(
            StudentAlreadyRegisteredException exception) {
    ...
    ResponseEntity<ApiError> handleStudentInviteNotFound(StudentInviteNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(
                        ApiError.of(
                                "STUDENT_INVITE_NOT_FOUND",
                                "Student invitation not found",
                                MDC.get("traceId")));
    }

    @ExceptionHandler(StudentInviteAlreadyAcceptedException.class)
    ResponseEntity<ApiError> handleStudentInviteAlreadyAccepted(
            StudentInviteAlreadyAcceptedException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
    ...
    ResponseEntity<ApiError> handlePublicStudentInviteAlreadyAccepted(
        ...
        return gone(
                "STUDENT_INVITE_ALREADY_ACCEPTED", "Student invitation has already been accepted");
    }
    ...
    ResponseEntity<ApiError> handleStudentInviteEmailConflict(
            StudentInviteEmailConflictException exception) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(
                        ApiError.of(
