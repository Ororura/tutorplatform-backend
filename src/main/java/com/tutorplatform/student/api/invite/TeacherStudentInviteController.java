...
public class TeacherStudentInviteController implements TeacherStudentInviteApi {
    ...
    @PostMapping
    @Override
    public ResponseEntity<StudentInviteCreatedResponse> createInvite(
            @AuthenticationPrincipal AuthenticatedUser principal,
            @PathVariable UUID studentId,
            @Valid @RequestBody CreateStudentInviteRequest request) {
        StudentInviteCreatedResponse response =
                studentInviteService.createInvite(principal, studentId, request);
    ...
    @GetMapping
    @Override
    public StudentInviteListResponse listInvites(
