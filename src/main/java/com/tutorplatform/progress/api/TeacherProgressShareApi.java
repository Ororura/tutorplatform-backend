...
public interface TeacherProgressShareApi {
    ...
    @Operation(operationId = "createProgressShare", summary = "Create a live current-progress share")
    @ApiResponses({
    ...
    ProgressShareListResponse listProgressShares(
        @Parameter(hidden = true) AuthenticatedUser principal,
        UUID studentId,
        @Parameter(description = "Optional student-program filter", schema = @Schema(format = "uuid"))
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "401", description = "Authentication required", content = @Content(schema = @Schema(implementation = ApiError.class))),
        @ApiResponse(responseCode = "403", description = "Teacher role required or CSRF rejected", content = @Content(schema = @Schema(implementation = ApiError.class))),
        @ApiResponse(responseCode = "404", description = "Student or progress share not found", content = @Content(schema = @Schema(implementation = ApiError.class)))
    })
