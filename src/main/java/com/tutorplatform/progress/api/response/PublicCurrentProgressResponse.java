...
public record PublicCurrentProgressResponse(
    ...
    long sessionsCount,
    @Schema(description = "Attendance ratio in the 0..1 range", example = "0.75") double attendanceRate,
    ...
    public static PublicCurrentProgressResponse from(CurrentProgress progress) {
        return new PublicCurrentProgressResponse(
            ...
            new PublicTopicsResponse(
                ...
                progress.inProgressTopics().stream().map(PublicTopicResponse::from).toList()
            ),
            new PublicHomeworkResponse(progress.homeworkAssigned(), progress.homeworkCompleted()),
            new PublicPracticeResponse(progress.practiceAssigned(), progress.practiceCompleted()),
    ...
    public record PublicTopicsResponse(
        List<PublicTopicResponse> completed,
        List<PublicTopicResponse> inProgress
    ) {
    }
